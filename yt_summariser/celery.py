...
try:
    ...
    def _task_prerun_handler(sender=None, task_id=None, task=None, args=None, kwargs=None, **extra):
        try:
            ...
            if not name.startswith('video_processor.'):
                return
            if name in ('video_processor.log_stage_duration', 'video_processor.log_pipeline_total'):
                return
            ts = time.time()
            with _task_lock:
    ...
    def _task_postrun_handler(sender=None, task_id=None, task=None, args=None, kwargs=None, retval=None, **extra):
        try:
            name = getattr(task, 'name', '') or str(sender)
            if not name.startswith('video_processor.'):
                return
    ...
    def _task_failure_handler(sender=None, task_id=None, exception=None, args=None, kwargs=None, einfo=None, **extra):
        try:
            name = str(sender)
            if not name.startswith('video_processor.'):
                return
