...
async def process_single_video(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        url_request = await sync_to_async(URLRequestTable.objects.create)(
            url=str(video_request.url), ip_address=ip_address, status="processing"
        )
        ...
        stream_url = reverse(
            "video_status_stream", kwargs={"request_id": url_request.request_id}
        )
        ...
        logger.info(
            f"Video processing initiated for URL: {str(video_request.url)[:50]}..."
        )
        return HttpResponse(
            response_data.model_dump_json(), content_type="application/json", status=201
        )
...
async def get_video_summary(request: HttpRequest, request_id: UUID) -> JsonResponse:
    ...
    try:
            ...
            processing_response = {
                ...
                "stages": {
                    ...
                    "summary_generated": bool(
                        transcript.summary and transcript.summary.strip()
                    ),
                },
            }
        ...
        video_metadata_formatted = response_service.format_video_metadata(
            video_metadata
        )
        ...
        try:
            if transcript.summary:
                ...
                logger.debug(
                    f"Raw summary type: {type(raw_summary)}, length: {len(str(raw_summary)) if raw_summary else 0}"
                )
                ...
                if isinstance(raw_summary, bytes):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to decode binary summary for video {transcript.video_id}"
                        )
                        summary_text = ""
                elif isinstance(raw_summary, dict):
                    # Handle JSON object - extract text content
                    logger.warning(
                        f"Summary is dict for video {transcript.video_id}, extracting text"
                    )
                    summary_text = str(
                        raw_summary.get(
                            "text", raw_summary.get("summary", str(raw_summary))
                        )
                    ).strip()
                elif isinstance(raw_summary, list):
                    # Handle array - join elements
                    logger.warning(
                        f"Summary is list for video {transcript.video_id}, joining elements"
                    )
                    summary_text = " ".join(
                        str(item) for item in raw_summary if item
                    ).strip()
                else:
                ...
                if not summary_text:
                    logger.warning(
                        f"Summary converted to empty string for video {transcript.video_id}"
                    )
                elif len(summary_text) < 10:
                    logger.warning(
                        f"Summary too short for video {transcript.video_id}: {len(summary_text)} chars"
                    )
                ...
                if any(indicator in summary_lower for indicator in error_indicators):
                    logger.warning(
                        f"Summary contains error message for video {transcript.video_id}: {summary_text[:100]}..."
                    )
                    error_response = response_service.format_error_response(
                    ...
                    logger.warning(
                        f"Summary contains null-like value for video {transcript.video_id}: {summary_text}"
                    )
                    summary_text = ""
            ...
            logger.error(
                f"Error processing summary for video {transcript.video_id}: {e}"
            )
            summary_text = ""
        ...
        if not summary_text or len(summary_text.strip()) == 0:
            logger.warning(
                f"Final summary validation failed for video {transcript.video_id}"
            )
            error_response = response_service.format_error_response(
                error_type="Summary not available",
        ...
        try:
            ...
            logger.debug(
                f"Raw key_points type: {type(raw_key_points)}, is_none: {raw_key_points is None}"
            )

            # Log sample of raw data for debugging (truncated for security)
            if raw_key_points is not None:
                sample_data = (
                    str(raw_key_points)[:200]
                    if len(str(raw_key_points)) > 200
                    else str(raw_key_points)
                )
                logger.debug(
                    f"Key points data sample for video {transcript.video_id}: {sample_data}"
                )
            ...
            if raw_key_points is None:
                ...
                # Handle binary data
                logger.warning(
                    f"key_points is bytes for video {transcript.video_id}, attempting to decode"
                )
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Successfully decoded and parsed binary key_points for video {transcript.video_id}"
                        )
                    except json.JSONDecodeError:
                        # Treat as plain text
                        simple_key_points = (
                            [decoded_data.strip()] if decoded_data.strip() else []
                        )
                        chapters_data = None
                except UnicodeDecodeError:
                    logger.error(
                        f"Failed to decode binary key_points for video {transcript.video_id}"
                    )
                    simple_key_points = []
                    chapters_data = None
                ...
                logger.warning(
                    f"key_points is string for video {transcript.video_id}, attempting to parse"
                )
                ...
                if raw_key_points_stripped.startswith(("[", "{")):
                    try:
                        ...
                        logger.debug(
                            f"Successfully parsed JSON string key_points for video {transcript.video_id}"
                        )
                    except json.JSONDecodeError as je:
                        logger.error(
                            f"Failed to parse JSON string key_points for video {transcript.video_id}: {je}"
                        )
                        # Treat as plain text key point
                        simple_key_points = (
                            [raw_key_points_stripped] if raw_key_points_stripped else []
                        )
                        chapters_data = None
                else:
                    # Treat as plain text key point
                    logger.debug(
                        f"Treating string key_points as plain text for video {transcript.video_id}"
                    )
                    simple_key_points = (
                        [raw_key_points_stripped] if raw_key_points_stripped else []
                    )
                ...
                logger.warning(
                    f"key_points is dict for video {transcript.video_id}, attempting to process"
                )
                ...
                try:
                    if "key_points" in raw_key_points and isinstance(
                        raw_key_points["key_points"], list
                    ):
                        extracted_key_points = raw_key_points["key_points"]
                        logger.debug(
                            f"Extracted key_points from wrapper dict for video {transcript.video_id}"
                        )
                    if "chapters" in raw_key_points and isinstance(
                        raw_key_points["chapters"], list
                    ):
                        extracted_chapters = raw_key_points["chapters"]
                        logger.debug(
                            f"Extracted chapters from wrapper dict for video {transcript.video_id}"
                        )
                except Exception:
                    pass
                ...
                if extracted_key_points is not None:
                    ...
                    logger.debug(
                        f"Treating dict as single chapter for video {transcript.video_id}"
                    )
            ...
            if isinstance(raw_key_points, list):
                if not raw_key_points:
                    # Empty list
                    logger.info(
                        f"Empty key_points list for video {transcript.video_id}"
                    )
                    simple_key_points = []
                    chapters_data = None
                    ...
                    dict_count = sum(
                        1 for item in raw_key_points if isinstance(item, dict)
                    )
                    string_count = sum(
                        1 for item in raw_key_points if isinstance(item, str)
                    )
                    other_count = len(raw_key_points) - dict_count - string_count

                    logger.debug(
                        f"Key points analysis for video {transcript.video_id}: "
                    ...
                    if dict_count > 0 and dict_count >= string_count:
                        ...
                        logger.info(
                            f"Processing structured chapters for video {transcript.video_id}: {dict_count} chapter objects"
                        )
                        ...
                        for i, chapter in enumerate(raw_key_points):
                            if not isinstance(chapter, dict):
                                logger.warning(
                                    f"Skipping non-dict item at index {i} for video {transcript.video_id}: {type(chapter)}"
                                )
                                continue
                            ...
                            try:
                                validated_chapter = {
                                    ...
                                    "title": str(
                                        chapter.get(
                                            ...
                                            chapter.get(
                                                "heading",
                                                chapter.get("name", f"Chapter {i + 1}"),
                                            ),
                                        )
                                    ).strip(),
                                    "summary": str(
                                ...
                                if not validated_chapter["title"] or validated_chapter[
                                    "title"
                                ].lower() in ["null", "none", "undefined"]:
                                    validated_chapter["title"] = f"Chapter {i + 1}"
                                elif len(validated_chapter["title"]) > 300:
                                    validated_chapter["title"] = (
                                        validated_chapter["title"][:297] + "..."
                                    )

                                # Validate and clean summary
                                if not validated_chapter[
                                    "summary"
                                ] or validated_chapter["summary"].lower() in [
                                    "null",
                                    "none",
                                    "undefined",
                                ]:
                                    validated_chapter["summary"] = (
                                        "No summary available"
                                    )
                                elif len(validated_chapter["summary"]) > 2000:
                                    validated_chapter["summary"] = (
                                        validated_chapter["summary"][:1997] + "..."
                                    )

                                # Validate chapter number
                                try:
                                    chapter_num = int(validated_chapter["chapter"])
                                    validated_chapter["chapter"] = max(
                                        1, chapter_num
                                    )  # Ensure positive
                                ...
                                logger.warning(
                                    f"Error processing chapter {i} for video {transcript.video_id}: {ce}"
                                )
                                continue

                        chapters_data = (
                            validated_chapters if validated_chapters else None
                        )
                        logger.debug(
                            f"Processed {len(validated_chapters)} structured chapters"
                        )

                    else:
                        # Simple string list format or mixed content
                        logger.info(
                            f"Processing as simple key points for video {transcript.video_id}: {len(raw_key_points)} items"
                        )
                        ...
                        for i, point in enumerate(raw_key_points):
                            try:
                                ...
                                if isinstance(point, dict):
                                    ...
                                    str_point = str(
                                        point.get(
                                            "title",
                                            point.get(
                                                "text", point.get("content", str(point))
                                            ),
                                        )
                                    ).strip()
                                else:
                                ...
                                if (
                                    str_point
                                    and str_point.lower()
                                    not in ["null", "none", "undefined", "nan", "{}"]
                                    and len(str_point) > 2
                                ):
                                ...
                                logger.warning(
                                    f"Error processing key point {i} for video {transcript.video_id}: {pe}"
                                )
                                continue

                        chapters_data = None
                        logger.debug(
                            f"Processed {len(simple_key_points)} simple key points"
                        )

            else:
                # Unexpected data type after all processing attempts
                logger.error(
                    f"Unexpected key_points type after processing for video {transcript.video_id}: {type(raw_key_points)}"
                )

                # Final fallback - try to extract something useful
                try:
                    fallback_text = str(raw_key_points).strip()
                    if fallback_text and fallback_text.lower() not in [
                        "null",
                        "none",
                        "undefined",
                    ]:
            ...
            logger.error(
                f"Critical error processing key_points for video {transcript.video_id}: {e}",
                exc_info=True,
            )
        ...
        logger.info(
            f"Serialization complete for video {transcript.video_id}: "
        ...
        if chapters_data:
            logger.debug(
                f"Chapters data preview for video {transcript.video_id}: "
        ...
        if simple_key_points:
            logger.debug(
                f"Key points preview for video {transcript.video_id}: "
        ...
        try:
            ...
            if not summary_text or len(summary_text.strip()) < 5:
                logger.error(
                    f"Summary validation failed at final stage for video {transcript.video_id}"
                )
                error_response = response_service.format_error_response(
            ...
            if simple_key_points:
                ...
                if len(valid_key_points) != len(simple_key_points):
                    logger.warning(
                        f"Cleaned {len(simple_key_points) - len(valid_key_points)} invalid key points for video {transcript.video_id}"
                    )
            ...
            if chapters_data:
                ...
                for chapter in chapters_data:
                    if (
                        ...
                        and len(chapter.get("title", "").strip()) > 0
                    ):
                        valid_chapters.append(chapter)

                if len(valid_chapters) != len(chapters_data):
                    logger.warning(
                        f"Cleaned {len(chapters_data) - len(valid_chapters)} invalid chapters for video {transcript.video_id}"
                    )
            ...
            if (
                not chapters_data
                and hasattr(transcript, "chapters")
                and isinstance(transcript.chapters, list)
                and transcript.chapters
            ):
                chapters_data = transcript.chapters

            # Prepare validated response with comprehensive error handling
            logger.debug(
                f"Creating VideoSummaryResponse for video {transcript.video_id}"
            )
            ...
            try:
                ...
                logger.debug(
                    f"Response JSON created successfully for video {transcript.video_id}"
                )

            except Exception as json_error:
                logger.error(
                    f"JSON serialization test failed for video {transcript.video_id}: {json_error}"
                )
                error_response = response_service.format_error_response(
            ...
            logger.error(
                f"Pydantic validation failed for video {transcript.video_id}: {ve}"
            )
            ...
            logger.error(
                f"Unexpected error during response preparation for video {transcript.video_id}: {e}",
                exc_info=True,
            )
            error_response = response_service.format_error_response(
