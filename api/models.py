...
class UnifiedSession(models.Model):
    ...
    video_requests = models.PositiveSmallIntegerField(
        default=0, help_text="Number of single video processing requests"
    )
    playlist_requests = models.PositiveSmallIntegerField(
        default=0, help_text="Number of playlist processing requests"
    )
    topic_requests = models.PositiveSmallIntegerField(
        default=0, help_text="Number of topic search requests"
    )
    ...
    def increment_request_count(self, request_type):
        ...
        updates = {field: F(field) + 1, "last_request_at": timezone.now()}
