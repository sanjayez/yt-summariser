...
class ResponseService:
    ...
    def compress_context(
        self, results: list[dict[str, Any]], max_chars: int = 600
    ) -> str:
        """
        ...
        result_text = "\n".join(compressed)
        self.logger.debug(
            f"Compressed {len(results)} results to {len(result_text)} chars"
        )
    ...
    def format_video_metadata(self, video_metadata) -> dict[str, Any]:
        ...
        return {
            ...
            "channel_thumbnail": (
                str(vm.channel_thumbnail) if vm.channel_thumbnail else None
            ),
    ...
    def format_search_sources(
        self, results: list[dict[str, Any]], video_metadata=None
    ) -> list[dict[str, Any]]:
        """
                ...
                formatted_sources.append(
                    {
                        ...
                        "youtube_url": metadata.get(
                            "youtube_url",
                            str(video_metadata.webpage_url) if video_metadata else "",
                        ),
    ...
    def format_error_response(
        ...
        """
        response = {"error": error_type, "status": status, "message": message}
    ...
    def format_timing_info(
        self, stage_timings: dict[str, float], total_time: float | None = None
    ) -> dict[str, Any]:
        """
