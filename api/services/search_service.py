...
from telemetry import TimingContext, get_logger, handle_exceptions, timed_operation
...
class SearchService:
    ...
    async def search_video_content(
        self, question: str, video_metadata, transcript
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            with TimingContext("context_processing") as timer:
                (
                    processed_context,
                    formatted_sources,
                ) = await self._process_search_results(search_results, video_metadata)
            ...
            if stage_timings:
                self.logger.error(
                    f"RAG pipeline error after {len(stage_timings)} stages"
                )
    ...
    async def _perform_vector_search(
        self, question: str, video_id: str, services: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            self.logger.info(
                f"🔍 Starting dual vector search for video {video_id} with question: '{question}'"
            )
            ...
            async def _search_chunks():
                self.logger.info("🔍 Searching transcript chunks...")
            ...
            async def _search_segments():
                self.logger.info("🔍 Searching segments...")
                return await vector_service.search_by_text(
                    text=question,
                    top_k=5,
            ...
            chunks_results, segments_results = await _asyncio.gather(
                _search_chunks(), _search_segments(), return_exceptions=False
            )

            # Process chunks results (for LLM context)
            chunks_count = (
                len(chunks_results.results)
                if chunks_results and chunks_results.results
                else 0
            )
            segments_count = (
                len(segments_results.results)
                if segments_results and segments_results.results
                else 0
            )

            self.logger.info(
                f"🔍 Dual search completed. Chunks: {chunks_count}, Segments: {segments_count}"
            )

            # Process transcript chunks (priority for answer generation)
            if chunks_results and chunks_results.results:
                self.logger.info(
                    f"🔍 Processing {len(chunks_results.results)} transcript chunks"
                )
                for i, result in enumerate(chunks_results.results):
                    raw_score = getattr(result, "score", None)
                    metadata_score = (
                        getattr(result.metadata, "score", None)
                    ...
                    final_score = (
                        ...
                        else (metadata_score if metadata_score is not None else 0.0)
                    )

                    self.logger.info(
                        f"🔍 Chunk {i + 1}: score={final_score:.4f}, text='{result.text[:50]}...'"
                    )
            ...
            if segments_results and segments_results.results:
                self.logger.info(
                    f"🔍 Processing {len(segments_results.results)} segments"
                )
                for i, result in enumerate(segments_results.results):
                    ...
                    self.logger.info(
                        f"🔍 Segment {i + 1}: score={final_score:.4f}, text='{result.text[:50]}...'"
                    )
            ...
            if not chunks_count and not segments_count:
                self.logger.warning(
                    f"🔍 No vector search results found for video {video_id}"
                )
            elif chunks_count and not segments_count:
                self.logger.info("🔍 Strategy: Chunks-only (no segments found)")
            elif not chunks_count and segments_count:
                self.logger.info(
                    "🔍 Strategy: Segments-only (no chunks found - fallback mode)"
                )
            else:
                self.logger.info(
                    "🔍 Strategy: Hybrid (chunks for context, segments for navigation)"
                )
    ...
    async def _process_search_results(
        self, search_results: list[dict[str, Any]], video_metadata
        ...
        self.logger.info(
            f"🔍 Processing results: {len(chunks)} chunks, {len(segments)} segments"
        )
        ...
        sources_data = (
            segments if segments else chunks
        ...
        self.logger.info(
            f"🔍 Context strategy: Top {len(top_context)} highest-scoring results (mixed chunks+segments)"
        )
        self.logger.info(
            f"🔍 Sources strategy: {len(top_sources)} items from {'segments' if segments else 'chunks'}"
        )

        # Generate LLM context from optimal source
        compressed_context = self.response_service.compress_context(
            top_context, max_chars=1800
        )  # Enhanced coverage for better keyword inclusion

        # Format user-facing sources from optimal source
        formatted_sources = self.response_service.format_search_sources(
            top_sources, video_metadata
        )

        return compressed_context, formatted_sources

    async def _generate_answer(
        self, question: str, context: str, services: dict[str, Any]
    ) -> str:
        """
...
Answer:"""
        ...
        try:
            ...
            if (
                gemini_response
                and hasattr(gemini_response, "choices")
                and gemini_response.choices
            ):
    ...
    async def _assemble_response(
        ...
        self.logger.debug(
            f"RAG pipeline completed in {total_time:.2f}ms with {len(stage_timings)} stages"
        )

        return response

    @handle_exceptions(reraise=True)
    async def search_transcript_fallback(
        self, question: str, transcript_text: str, video_metadata
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if top_sentences:
                ...
                avg_confidence = sum(s["relevance"] for s in top_sentences) / len(
                    top_sentences
                )
