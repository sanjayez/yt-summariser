...
class UnifiedProcessRequest(BaseModel):
    ...
    type: Literal["video", "playlist", "topic"] = Field(
        ..., description="Type of processing request"
    )
    ...
    class Config:
        json_schema_extra = {
            "examples": [
                ...
                {"content": "python machine learning tutorial", "type": "topic"},
            ]
        }
...
class VideoQuestionRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"question": "What are the main topics discussed in this video?"}
        }
...
class SearchSource(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score for this source"
    )
...
class VideoMetadataResponse(BaseModel):
    ...
    duration_string: str | None = Field(None, description="Duration in MM:SS format")

    # Channel
    channel_name: str | None = Field(None, description="Channel name")
    channel_id: str | None = Field(None, description="YouTube channel ID")
    uploader_id: str | None = Field(None, description="Uploader handle")
    channel_thumbnail: HttpUrl | None = Field(
        None, description="Channel avatar/thumbnail URL"
    )

    # Media
    thumbnail: HttpUrl | None = Field(None, description="Video thumbnail URL")
    youtube_url: HttpUrl = Field(..., description="YouTube video URL")
    ...
    channel_follower_count: int | None = Field(
        None, description="Channel subscriber count"
    )
    channel_is_verified: bool | None = Field(
        None, description="Whether the channel is verified"
    )

    # Other
    language: str | None = Field(None, description="Video language")
    upload_date: str | None = Field(
        None, description="Upload date in YYYY-MM-DD format"
    )
    tags: list[str] | None = Field(None, description="Video tags")
    categories: list[str] | None = Field(None, description="Categories")
    engagement: list[dict[str, Any]] | None = Field(
        None, description="High engagement segments"
    )
    is_embedded: bool | None = Field(
        None, description="Whether metadata is embedded in vector store"
    )
    status: str | None = Field(None, description="Processing status of metadata")
    created_at: str | None = Field(None, description="Created at timestamp (ISO)")
...
class ProcessingStages(BaseModel):
    ...
    transcript_extracted: bool = Field(
        ..., description="Transcript extraction completed"
    )
...
class TimingInfo(BaseModel):
    ...
    total_time_ms: float | None = Field(None, description="Total time in milliseconds")
    stage_timings: dict[str, float] | None = Field(
        None, description="Individual stage timings"
    )
    measured_stages: int | None = Field(None, description="Number of measured stages")
    note: str | None = Field(None, description="Additional timing notes")
...
class VideoProcessResponse(BaseModel):
    ...
    stream_url: str = Field(
        ...,
        description="Relative SSE path for this request (e.g., /api/video/status/<request_id>/)",
    )
...
class VideoSummaryResponse(BaseModel):
    ...
    chapters: list[dict[str, Any]] | None = Field(
        None, description="Chapter-wise structured content"
    )
    video_metadata: VideoMetadataResponse = Field(..., description="Video metadata")
    status: str = Field(..., description="Processing status")
    generated_at: str | None = Field(
        None, description="Summary generation timestamp in ISO format"
    )
...
class VideoSearchResponse(BaseModel):
    ...
    video_metadata: dict[str, Any] | None = Field(None, description="Video metadata")
    timing: TimingInfo | None = Field(
        None, description="Performance timing information"
    )
...
class VideoStatusResponse(BaseModel):
    ...
    overall_status: VideoProcessingStatus = Field(
        ..., description="Overall processing status"
    )
    timestamp: float = Field(..., description="Unix timestamp")
    metadata_status: str | None = Field(None, description="Metadata extraction status")
    transcript_status: str | None = Field(
        None, description="Transcript extraction status"
    )
    summary_status: str | None = Field(None, description="Summary generation status")
    embedding_status: str | None = Field(
        None, description="Embedding generation status"
    )
    stages: ProcessingStages = Field(
        ..., description="Processing stage completion status"
    )
