...
class URLRequestTableSerializer(serializers.ModelSerializer):
    ...
    status = serializers.ChoiceField(
        choices=URLRequestTable.STATUS_CHOICES, default="processing", read_only=True
    )
    ...
    celery_task_id = serializers.CharField(
        max_length=255, read_only=True, allow_null=True
    )
    chain_task_id = serializers.CharField(
        max_length=255, read_only=True, allow_null=True
    )
    failure_reason = serializers.ChoiceField(
        choices=URLRequestTable.FAILURE_REASON_CHOICES, read_only=True, allow_null=True
    )
...
class URLRequestStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = URLRequestTable
        fields = ["request_id", "status", "failure_reason", "created_at"]
