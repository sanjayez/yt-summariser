...
from telemetry import TimingContext, get_logger
...
class APISearchWorkflow(Workflow):
    ...
    async def perform_vector_search(
        self, ctx: Context, ev: SearchInitEvent
    ) -> VectorSearchEvent:
        """
        ...
        async with TimingContext("vector_search") as timer:
            try:
                ...
                self.logger.info(
                    f"🔍 Performing vector search: '{ev.question[:50]}...'"
                )
                ...
                if segments_results and segments_results.results:
                    self.logger.info(
                        f"🔍 Found {len(segments_results.results)} vector results"
                    )
                    ...
                    self.logger.warning(
                        f"🔍 No vector results found for video {ev.video_id}"
                    )
    ...
    @step
    async def process_search_context(
        self, ctx: Context, ev: VectorSearchEvent
    ) -> ContextProcessedEvent:
        """
    ...
    async def generate_answer(
        self, ctx: Context, ev: ContextProcessedEvent
...
Answer:"""
                    ...
                    response = await llm_service.provider.chat_completion(
                        gemini_request
                    )
    ...
    async def finalize_response(
        self, ctx: Context, ev: AnswerGeneratedEvent
    ) -> StopEvent:
        """
        ...
        async with TimingContext("response_finalization") as timer:
            ...
            confidence = self.response_service.calculate_confidence_score(
                ev.search_results
            )
            ...
            timing_info = self.response_service.format_timing_info(
                ctx.data["timings"], sum(ctx.data["timings"].values())
            )
    ...
    async def run_search(
        ...
        result = await self.run(
            question=question, video_metadata=video_metadata, transcript=transcript
        )
