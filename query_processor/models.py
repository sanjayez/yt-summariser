...
class QueryRequest(models.Model):
    ...
    video_urls = models.JSONField(
        default=list, blank=True, help_text="List of YouTube video URLs found/processed"
    )

    total_videos = models.PositiveIntegerField(
        default=0, help_text="Total number of videos found/processed"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When this search request was created"
    )
        ...
        constraints = [
            models.CheckConstraint(
                check=~(
                    ...
                    & (
                        models.Q(error_message__isnull=True)
