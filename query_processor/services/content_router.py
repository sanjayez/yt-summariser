...
class ContentRouterService:
    ...
    async def route_request(
        unified_session, request_type: str, content: str
    ) -> dict[str, Any]:
        logger.info(
            f"🔀 Routing {request_type} request for session {unified_session.session_id}"
        )
    ...
    async def _route_video_request(query_request: QueryRequest) -> dict[str, Any]:
        try:
            ...
            return {
                ...
                if query_request.video_urls
                else query_request.original_content,
            ...
            error_msg = (
                f"Failed to route {query_request.request_type} request: {str(e)}"
            )

            # Update QueryRequest with error
            await ContentRouterService._update_query_request_error(
                query_request, error_msg
            )
    ...
    @staticmethod
    async def _update_query_request_error(
        query_request: QueryRequest, error_message: str
    ):
        try:
            ...
            logger.debug(
                "Updated QueryRequest %s with error status", query_request.search_id
            )
        except Exception:
            logger.exception(
