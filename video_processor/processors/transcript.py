...
from ..config import TRANSCRIPT_CONFIG, YOUTUBE_CONFIG
...
from ..utils.language_detection import detect_transcript_language, get_api_language
...
def extract_transcript_with_decodo(video_id, primary_language="en"):
    ...
    if success and result.get("segments"):
        logger.info(
            f"Successfully extracted transcript using language: {primary_language}"
        )
    ...
    for lang_code in language_variants[1:]:  # Skip first one (already tried)
        try:
            ...
            if success and result.get("segments"):
                logger.info(
                    f"Successfully extracted transcript using language variant: {lang_code}"
                )
    ...
    remaining_languages = [
        lang for lang in fallback_languages if lang not in language_variants
    ]
    ...
    for lang_code in remaining_languages:
        try:
            ...
            if success and result.get("segments"):
                logger.info(
                    f"Successfully extracted transcript using fallback language: {lang_code}"
                )
                return result["segments"], lang_code
        except Exception as e:
    ...
    raise Exception(
        f"No transcript found for video {video_id} in any supported language"
    )


def extract_transcript_with_fallback(
    ...
    if TRANSCRIPT_CONFIG["DECODO"]["enabled"]:
        try:
            ...
            transcript_data, used_language = extract_transcript_with_decodo(
                video_id, language
            )
    ...
    if (
        TRANSCRIPT_CONFIG["YOUTUBE_API"]["enabled"]
        and TRANSCRIPT_CONFIG["FALLBACK_STRATEGY"]["enable_fallback"]
    ):
        try:
            logger.info(f"Attempting YouTube API fallback extraction for {video_id}")

            preferred_languages = TRANSCRIPT_CONFIG["YOUTUBE_API"][
                "preferred_languages"
            ]
            ...
            success, result = extract_youtube_transcript_fallback(
    ...
    raise Exception(
        f"No transcript available for video {video_id} through any extraction method"
    )
...
def extract_video_transcript(self, metadata_result, url_request_id):
    ...
    if metadata_result and metadata_result.get("excluded"):
        logger.info(
            f"Video was excluded in metadata stage: {metadata_result.get('exclusion_reason')}"
        )
    ...
    try:
        progress_recorder.set_progress(
            0, 100, description="Starting transcript extraction"
        )
        ...
        logger.info(
            f"Starting transcript extraction for video {video_id} using Decodo API"
        )
        ...
        progress_recorder.set_progress(
            10, 100, description="Initializing transcript extraction"
        )
        ...
        progress_recorder.set_progress(
            20, 100, description="Extracting transcript from Decodo API"
        )
        ...
        logger.info(
            f"Video language: {detected_language}, API language: {api_language}, using base language: {base_language}"
        )

        # Use fallback extraction strategy
        transcript_result, used_language, extraction_source = (
            extract_transcript_with_fallback(video_id, base_language)
        )

        logger.info(f"Transcript extracted using {extraction_source} for {video_id}")
        logger.info(
            f"Extracted transcript with {len(transcript_result.get('segments', []))} segments using language: {used_language}"
        )

        progress_recorder.set_progress(
            60, 100, description="Validating transcript data"
        )
        ...
        if transcript_data:
            ...
            if validated_transcript:
                validated_text = " ".join(
                    [item["text"] for item in validated_transcript]
                )
                if validated_text:
        ...
        progress_recorder.set_progress(
            70, 100, description="Saving transcript to database"
        )
        ...
        with transaction.atomic():
            ...
            if detected_language and detected_language.startswith("fallback-"):
                logger.info(
                    f"Performing language detection for fallback language: {detected_language}"
                )
                ...
                if is_english:
                    ...
                    video_metadata.save()
                    logger.info(
                        f"Language updated: {detected_language} → {updated_language} (confidence: {confidence:.3f})"
                    )
                else:
                    # Non-English content detected - log for classification stage
                    logger.info(
                        f"Non-English content detected (confidence: {confidence:.3f} < threshold)"
                    )
                    logger.info(
                        f"Video {video_id} language classification will be handled at final stage"
                    )
        ...
        progress_recorder.set_progress(
            100, 100, description="Transcript extraction complete"
        )
        ...
        logger.info(
            f"Successfully extracted transcript with {len(transcript_data)} segments using {extraction_source}"
        )
        ...
        return {
            ...
            "video_title": metadata_result.get("title", "Unknown")
            if isinstance(metadata_result, dict)
            else video_metadata.title or "Unknown",
        }

    except SoftTimeLimitExceeded:
        # Task is approaching timeout - save status and exit gracefully
        logger.warning(
            f"Transcript extraction soft timeout reached for video {video_id or 'unknown'}"
        )
        ...
        try:
            ...
            if video_metadata and video_id:
                ...
                logger.error(
                    f"Marked transcript extraction as failed due to timeout: {video_id}"
                )

        except Exception as cleanup_error:
            logger.error(
                f"Failed to update transcript status during timeout cleanup: {cleanup_error}"
            )

        # Re-raise with specific timeout message
        raise Exception(
            f"Transcript extraction timeout for video {video_id or 'unknown'}"
        )
        ...
        if self.request.retries >= self.max_retries:
            handle_dead_letter_task(
                "extract_video_transcript", self.request.id, [url_request_id], {}, e
            )
