    ...
    try:
        ...
        if hasattr(url_request, "video_metadata") and url_request.video_metadata:
            ...
            video_data["metadata"] = {
                ...
                "upload_date": str(video_metadata.upload_date)
            ...
            if (
                ...
                if (
                    hasattr(transcript, "content_analysis")
                    and transcript.content_analysis
                ):
                    ...
                    video_data["content_analysis"] = {
                        ...
                        "ad_duration_ratio": float(analysis.ad_duration_ratio)
                        if analysis.ad_duration_ratio is not None
                        else None,
                        "filler_duration_ratio": float(analysis.filler_duration_ratio)
                        if analysis.filler_duration_ratio is not None
                        else None,
                        "content_rating": float(analysis.content_rating)
                        if analysis.content_rating is not None
                        else None,
        ...
        logger.error(
            f"Error gathering complete video data for {url_request_id}: {e}",
            exc_info=True,
        )
        return {}
...
def update_overall_status(self, embedding_result, url_request_id):
    ...
    if embedding_result and embedding_result.get("excluded"):
        logger.info(
            f"Video was excluded - ensuring final status is correctly set: {embedding_result.get('exclusion_reason')}"
        )
        ...
        if url_request.status == "processing":
            ...
            url_request.save()
            logger.info(
                "Updated excluded video status from processing to failed/excluded"
            )
    ...
    try:
        ...
        if hasattr(url_request, "video_metadata") and url_request.video_metadata:
            try:
                ...
                if video_transcript and video_transcript.status == "failed":
                    ...
                    if isinstance(embedding_result, dict) and embedding_result.get(
                        "error"
                    ):
                ...
                if (
                    ...
                    and embedding_result.get("transcript_chunks_embedded", 0) == 0
                ):
        ...
        if not is_part_of_search:
            logger.info(
                f"Gathering complete video data for standalone video {url_request_id}"
            )
            video_data = _gather_complete_video_data(url_request_id)
        else:
            logger.info(
                f"Skipping video data gathering for search-related video {url_request_id}"
            )
        ...
        update_task_progress(
            ...
            100,
            {
                ...
                "has_transcript": hasattr(url_request, "video_metadata")
                and hasattr(url_request.video_metadata, "video_transcript"),
        ...
        result = {
            ...
            "transcript_status": getattr(
                url_request.video_metadata.video_transcript, "status", None
            )
            if hasattr(url_request, "video_metadata")
            and hasattr(url_request.video_metadata, "video_transcript")
            else None,
            "transcript_segments": embedding_result.get("transcript_chunks_embedded", 0)
            if isinstance(embedding_result, dict)
            else 0,
            "failure_reason": failure_reason if transcript_failed else None,
            "error_message": getattr(url_request.video_metadata, "error_message", None)
            if hasattr(url_request, "video_metadata")
            else None,
        }
        ...
        logger.error(
            f"Status update soft timeout reached for request {url_request_id} - this indicates a serious database issue"
        )
        ...
        try:
            ...
            logger.critical(
                f"Failed to emergency update status during timeout - database may be down: {cleanup_error}"
            )
        ...
        if self.request.retries >= self.max_retries:
            handle_dead_letter_task(
                "update_overall_status", self.request.id, [url_request_id], {}, e
            )
