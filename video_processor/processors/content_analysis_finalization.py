...
from .content_analyzer import add_timestamps_to_segments, calculate_content_ratios
...
def content_analysis_finalization(self, embedding_result, url_request_id):
    ...
    try:
        logger.info(
            f"Starting content analysis finalization for request {url_request_id}"
        )

        # Check if video was excluded in previous stage
        if embedding_result and embedding_result.get("excluded"):
            logger.info(
                f"Video was excluded in previous stage: {embedding_result.get('exclusion_reason')}"
            )
            return {
        ...
        if (
            not hasattr(video_transcript, "content_analysis")
            or not video_transcript.content_analysis
        ):
            # Create ContentAnalysis if it doesn't exist (shouldn't happen in normal flow)
            logger.warning(
                f"ContentAnalysis not found for {video_id}, creating empty record"
            )
        ...
        logger.info(
            f"Processing finalization for video {video_id}: {video_metadata.title}"
        )

        # Check if preliminary analysis was completed
        if not content_analysis.is_preliminary_complete:
            logger.warning(
                f"Preliminary analysis not complete for {video_id}, skipping finalization"
            )
        ...
        final_results = asyncio.run(
            add_timestamps_and_calculate_ratios_async(
        ...
        with transaction.atomic():
            ...
            content_analysis.filler_duration_ratio = final_results[
...
async def add_timestamps_and_calculate_ratios_async(
    content_analysis: ContentAnalysis, video_id: str, video_duration: int
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Vector search complete: {len(timestamped_ads)} ads, {len(timestamped_filler)} fillers with timestamps"
        )

        # Calculate timestamp counts
        ads_with_timestamps = sum(
            1 for seg in timestamped_ads if seg["start"] != 0.0 or seg["end"] != 0.0
        )
        fillers_with_timestamps = sum(
            1 for seg in timestamped_filler if seg["start"] != 0.0 or seg["end"] != 0.0
        )
        ...
        if video_duration > 0:
            ...
            for seg in timestamped_ads:
                all_segments.append(
                    {"start": seg["start"], "end": seg["end"], "type": "ad"}
                )

            for seg in timestamped_filler:
                all_segments.append(
                    {"start": seg["start"], "end": seg["end"], "type": "filler"}
                )
