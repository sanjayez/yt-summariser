...
def prepare_classification_context(
    ...
    if duration <= 0:
        logger.warning(
            f"Invalid or missing duration for video {video_metadata.video_id}"
        )
    ...
    logger.debug(
        f"Prepared classification context for video {video_metadata.video_id}: "
...
def parse_and_validate_classification(
    ...
    try:
        ...
        if result["exclusion_reason"] not in valid_exclusion_reasons:
            logger.warning(
                f"Invalid exclusion reason '{result['exclusion_reason']}' for video {video_id}, defaulting to background_music_only"
            )
            result["exclusion_reason"] = "background_music_only"

        # Validate primary_reason exists and is string
        if not result.get("primary_reason") or not isinstance(
            result["primary_reason"], str
        ):
            raise ValueError("primary_reason must be a non-empty string")

        logger.debug(
            f"Successfully parsed classification for video {video_id}: "
    ...
    try:
        ...
        messages = [
            ChatMessage(
                ...
                content="You are a precise video content classifier.",
            ),
        ...
        if result["status"] != "completed":
            raise Exception(
                f"LLM service error: {result.get('error', 'Unknown error')}"
            )
        ...
        logger.info(
            f"Tokens used: {response.usage.total_tokens if hasattr(response, 'usage') else 'unknown'}"
        )

        # Parse and validate response
        classification = parse_and_validate_classification(
            response.choices[0].message.content, video_id
        )
        ...
        logger.info(
            f"Video {video_id} classified as {classification['classification']} "
        ...
        logger.error(
            f"Context summary: title='{context.get('title', 'N/A')}', "
...
def classify_and_exclude_video_llm(self, previous_result, url_request_id):
    ...
    if previous_result and previous_result.get("excluded"):
        logger.info(
            f"Video was excluded in previous stage: {previous_result.get('exclusion_reason')}"
        )
    ...
    try:
        ...
        try:
            video_metadata = VideoMetadata.objects.get(
                url_request__request_id=url_request_id
            )
        ...
        logger.info(
            f"Starting LLM classification for video {video_metadata.video_id}: "
        ...
        try:
            ...
            logger.error(
                f"Context preparation failed for video {video_metadata.video_id}: {e}"
            )
            return {
                "video_id": video_metadata.video_id,
                "status": "failed",
        ...
        llm_should_exclude = (
            classification["classification"] in ["MUSIC_VIDEO", "BACKGROUND_ONLY"]
            and classification["confidence"] >= 0.7
            and not classification.get("error", False)
        )
        ...
        if llm_should_exclude:
            ...
            exclusion_reason = (
                classification["exclusion_reason"] or "background_music_only"
            )
            should_exclude = True
            logger.info(
                f"LLM exclusion decision for {video_metadata.video_id}: {exclusion_reason}"
            )
        else:
            # LLM said BUSINESS_SUITABLE - check language as final gate
            try:
                language_should_exclude, language_reason = should_exclude_for_language(
                ...
                if language_should_exclude:
                    ...
                    should_exclude = True
                    logger.info(
                        f"Language exclusion override for {video_metadata.video_id}: LLM said BUSINESS_SUITABLE but language check failed ({language_reason})"
                    )
                else:
                    should_exclude = False
                    logger.info(
                        f"Final decision for {video_metadata.video_id}: RETAINED (LLM: BUSINESS_SUITABLE, Language: {language_reason})"
                    )
            except Exception as e:
                logger.error(
                    f"Language exclusion check failed for {video_metadata.video_id}: {e}"
                )
        ...
        if should_exclude:
            ...
            logger.info(
                f"🚫 EXCLUDING video {video_metadata.video_id} for reason: {exclusion_reason}"
            )
            logger.info(
            ...
            try:
                added = add_video_to_exclusion_table(
                    video_metadata.url_request.url, exclusion_reason
                )

                if added:
                    logger.info(
                        f"✅ Successfully added video {video_metadata.video_id} to exclusion table: {exclusion_reason}"
                    )
                else:
                    logger.warning(
                        f"⚠️  Video {video_metadata.video_id} was NOT added to exclusion table (likely already exists)"
                    )

            except Exception as e:
                logger.error(
                    f"❌ Error adding video {video_metadata.video_id} to exclusion table: {e}"
                )
            ...
            try:
                ...
                logger.info(
                    f"✅ Updated URLRequest status: {old_status} → failed for video {video_metadata.video_id}"
                )
            except Exception as e:
                logger.error(
                    f"❌ Failed to update URLRequest status for video {video_metadata.video_id}: {e}"
                )
                logger.exception("Full traceback:")
            ...
            if classification.get("error", False):
                logger.warning(
                    f"NOT EXCLUDED due to classification error: {video_metadata.video_id}"
                )
            elif classification["confidence"] < 0.7:
                logger.warning(
                    f"NOT EXCLUDED due to low confidence ({classification['confidence']:.2f}): "
                    f"{video_metadata.video_id}"
                )
            else:
                logger.info(
                    f"RETAINED video {video_metadata.video_id}: {classification['classification']} "
        ...
        logger.debug(
            f"Classification completed for video {video_metadata.video_id}: {result}"
        )
        return result
        ...
        logger.error(
            f"Classification task failed for URL request {url_request_id}: {str(e)}"
        )
        logger.exception("Full traceback:")
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying classification for URL request {url_request_id} "
        ...
        logger.error(
            f"Classification permanently failed for URL request {url_request_id} "
