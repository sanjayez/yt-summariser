    ...
    try:
        ...
        try:
            url_request = URLRequestTable.objects.select_related(
                "search_request", "search_request__search_session"
            ).get(request_id=url_request_id)  # Use request_id (UUID)

            logger.info(
                f"Processing video {url_request.url} for search request {url_request.search_request.search_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Video already exists for URLRequest {url_request_id}, status: {existing_video.status}"
            )
            ...
            # If failed, we'll retry the processing
                ...
                logger.info(
                    f"Retrying failed video processing for URLRequest {url_request_id}"
                )
        ...
        _update_url_request_status(
            url_request, "processing", "Started video processing"
        )
        ...
        logger.warning(
            f"Search video processing soft timeout reached for URLRequest {url_request_id}"
        )
        ...
        try:
            ...
            if url_request:
                _update_url_request_status(
                    url_request, "failed", "Search video processing timeout"
                )
                logger.error(
                    f"Marked search video processing as failed due to timeout: {url_request_id}"
                )

        except Exception as cleanup_error:
            logger.error(
                f"Failed to update search video status during timeout cleanup: {cleanup_error}"
            )

        # Re-raise to mark task as failed
        raise Exception(
            f"Search video processing timeout for URLRequest {url_request_id}"
        )
        ...
        try:
            if url_request:
                _update_url_request_status(
                    url_request, "failed", f"Processing error: {str(e)}"
                )
        except:
            pass
...
def update_search_video_status(
    self, url_request_id: str, status: str, message: str = None
):
    """
    ...
    """
    logger.info(
        f"Updating search video status for URLRequest {url_request_id}: {status}"
    )
    ...
    try:
        ...
        try:
            url_request = URLRequestTable.objects.select_related("search_request").get(
        ...
        logger.info(
            f"Successfully updated URLRequest {url_request_id} status to {status}"
        )
        ...
        logger.error(
            f"Status update soft timeout reached for URLRequest {url_request_id} - database issue"
        )
        ...
        try:
            ...
            logger.critical(
                f"Failed to emergency update status during timeout: {cleanup_error}"
            )

        # Re-raise to mark task as failed
        raise Exception(
            f"Status update critical timeout for URLRequest {url_request_id}"
        )
...
def get_search_video_results(self, search_id: str):
    ...
    try:
        ...
        for url_request in url_requests:
            ...
            if hasattr(url_request, "video_metadata"):
                ...
                if hasattr(metadata, "video_transcript"):
                    ...
                    video_result["video_data"]["transcript_length"] = (
        ...
        logger.warning(
            f"Get search video results soft timeout reached for search {search_id}"
        )
...
def _update_url_request_status(
    url_request: URLRequestTable, status: str, message: str = None
):
    """
...
def _check_and_update_search_request_status(search_request: SearchRequestModel):
    ...
    try:
        ...
        logger.info(
            f"Updated search request {search_request.request_id} status: {status_message}"
        )
