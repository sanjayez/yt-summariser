...
from ..utils import handle_dead_letter_task, idempotent_task, update_task_progress
    ...
    try:
        ...
        if "CHAPTERS:" in structured_response:
            ...
            try:
                ...
                if isinstance(chapters_json, list) and len(chapters_json) > 0:
                    logger.info(
                        f"Successfully parsed {len(chapters_json)} chapters from CHAPTERS: section"
                    )
                    return chapters_json
            except json.JSONDecodeError:
                logger.warning(
                    "Failed to parse CHAPTERS section as JSON, trying cleanup"
                )
                ...
                try:
                    ...
                    if isinstance(chapters_json, list) and len(chapters_json) > 0:
                        logger.info(
                            f"Successfully parsed {len(chapters_json)} chapters after JSON cleanup"
                        )
                        return chapters_json
                except json.JSONDecodeError:
        ...
        for json_array in json_arrays:
            if len(json_array) > 0 and isinstance(json_array[0], dict):
                ...
                if any(key in json_array[0] for key in ["title", "summary", "chapter"]):
                    logger.info(
                        f"Found {len(json_array)} chapters via JSON array extraction"
                    )
                    return json_array
        ...
        if len(structured_response.strip()) > 50:  # Has substantial content
            fallback_chapter = {
                "chapter": 1,
                "title": "Video Summary",
                "summary": structured_response.strip()[:500]
                + ("..." if len(structured_response) > 500 else ""),
            }
...
def _parse_llm_response_robust(structured_response: str) -> tuple:
    ...
    try:
        ...
        if (
            "EXECUTIVE SUMMARY:" in structured_response
            and "CHAPTERS:" in structured_response
        ):
            try:
                ...
                if executive_summary and len(executive_summary.strip()) > 10:
                    logger.info(
                        "Successfully parsed structured response (perfect format)"
                    )
        ...
        if chapters:
            ...
            logger.info("Extracted summary using chapters-first strategy")
            return executive_summary, chapters
        ...
        return structured_response[
            :200
        ] if structured_response else "Parsing failed", []
...
def generate_summary_sync(transcript_text: str, video_metadata=None) -> tuple:
    ...
    try:
        ...
        if len(transcript_text) > 15000:  # More aggressive chunking for stability
            logger.info(
                f"Long transcript detected ({len(transcript_text)} chars), using enhanced chunking"
            )

            # Use smaller chunks for better stability
            chunks = chunk_transcript_text(
                transcript_text, chunk_size=2000, chunk_overlap=100
            )
            chunk_summaries = []
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    logger.info(
                        f"Processing chunk {i + 1}/{total_chunks} ({len(chunk)} chars)"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Chunk {i + 1} processed in {processing_time:.1f}s"
                        )

                        if (
                            response_data
                            and response_data.get("response")
                            and response_data["response"].choices
                        ):
                            chunk_summaries.append(
                                response_data["response"].choices[0].message.content
                            )
                            logger.info(
                                f"✅ Generated summary for chunk {i + 1}/{total_chunks}"
                            )
                        else:
                            logger.warning(
                                f"⚠️ Empty response for chunk {i + 1}, skipping"
                            )
            ...
            if chunk_summaries:
                ...
                logger.info(
                    f"Combined {len(chunk_summaries)}/{total_chunks} chunk summaries"
                )
...
Create a unified, well-structured summary with key points. Focus on the main themes and insights."""
                ...
                try:
                    ...
                    if (
                        final_response_data
                        and final_response_data.get("response")
                        and final_response_data["response"].choices
                    ):
                        structured_response = (
                            final_response_data["response"].choices[0].message.content
                        )
            ...
            executive_summary = validate_embedding_text(
                executive_summary, max_length=2000
            )

            logger.info(
                f"Chunked summary parsing completed: exec summary {len(executive_summary)} chars, {len(chapters)} chapters"
            )
            ...
            logger.info(
                f"Generating direct summary for transcript ({len(transcript_text)} chars)"
            )
            ...
            if (
                not response_data
                or not response_data.get("response")
                or not response_data["response"].choices
            ):
                with contextlib.suppress(Exception):
                    append_line(
                        file_path=os.getenv(
                            "TIMING_LOG_FILE", "logs/stage_timings.log"
                        ),
            ...
            try:
                preview = (
                    ...
                    if len(structured_response) > 200
        ...
        logger.info(
            f"Summary parsing completed: exec summary {len(executive_summary)} chars, {len(chapters)} chapters"
        )
...
def generate_video_summary(self, transcript_result, url_request_id):
    ...
    try:
        ...
        update_task_progress(
            self, TASK_STATES.get("GENERATING_SUMMARY", "Generating Summary"), 10
        )
        ...
        update_task_progress(
            self, TASK_STATES.get("GENERATING_SUMMARY", "Generating Summary"), 30
        )
        ...
        summary_text, key_points = generate_summary_sync(
            transcript_text, video_metadata
        )

        update_task_progress(
            self, TASK_STATES.get("GENERATING_SUMMARY", "Generating Summary"), 80
        )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Saved summary ({len(summary_text)} chars) and {len(key_points)} key points"
            )

        update_task_progress(
            self, TASK_STATES.get("GENERATING_SUMMARY", "Generating Summary"), 100
        )
        ...
        logger.info(
            f"Successfully generated summary for video {video_metadata.video_id}"
        )
        return result
        ...
        logger.warning(
            f"Summary generation soft timeout reached for request {url_request_id}"
        )
        ...
        try:
            ...
            if transcript:
                ...
                transcript.save()
                logger.error(
                    f"Marked summary generation as failed due to timeout: {video_metadata.video_id if video_metadata else 'unknown'}"
                )

        except Exception as cleanup_error:
            logger.error(
                f"Failed to update summary status during timeout cleanup: {cleanup_error}"
            )
        ...
        try:
            with transaction.atomic():
                ...
                if hasattr(url_request, "video_metadata") and hasattr(
                    url_request.video_metadata, "video_transcript"
                ):
        ...
        if self.request.retries >= self.max_retries:
            handle_dead_letter_task(
                "generate_video_summary", self.request.id, [url_request_id], {}, e
            )
