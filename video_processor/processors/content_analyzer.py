...
SUPPORTED_TONES = ['formal', 'informal', 'positive', 'negative', 'neutral', 'humorous', 'serious']


def chunk_transcript(transcript_text: str, max_chunk_size: int = 2000, overlap_sentences: int = 2) -> List[Dict]:
    """
    ...
    try:
        ...
        logger.info(f"Created {len(chunks)} chunks with {overlap_sentences}-sentence overlap")
        return chunks
            ...
            try:
                ...
                try:
                    ...
                    if json_match:
                        ...
                        logger.warning(f"Could not parse JSON from LLM response")
                        return {}
        ...
        if isinstance(result, dict) and 'tone' in result:
            ...
            if 'detected_tones' in tone_data:
                ...
                for tone_entry in tone_data['detected_tones']:
                    if isinstance(tone_entry, dict) and tone_entry.get('tone', '').lower() in SUPPORTED_TONES:
            ...
            if 'primary_tone' in tone_data:
                ...
                if primary not in SUPPORTED_TONES:
                    ...
                    if tone_data.get('detected_tones'):
                        tone_data['primary_tone'] = tone_data['detected_tones'][0]['tone']
                    else:
...
async def get_timestamps_for_text(text_excerpt: str, video_id: str) -> Tuple[float, float]:
    """
