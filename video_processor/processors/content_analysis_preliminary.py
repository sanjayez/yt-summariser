...
def content_analysis_preliminary(self, transcript_result, url_request_id):
    ...
    try:
        logger.info(f"Starting preliminary content analysis for request {url_request_id}")
        ...
        if not video_transcript.transcript_text or not video_transcript.transcript_text.strip():
            raise ValueError("No transcript text available")
        
        logger.info(f"Processing preliminary analysis for video {video_id}: {video_metadata.title}")
        ...
        with transaction.atomic():
            content_analysis, created = ContentAnalysis.objects.get_or_create(
                ...
                defaults={
                    'preliminary_analysis_status': 'processing'
                }
            )
        ...
        preliminary_results = asyncio.run(process_preliminary_analysis_async(
        ...
        with transaction.atomic():
            ...
            content_analysis.raw_filler_segments = preliminary_results['raw_filler_segments']
        ...
        return {
            ...
            'raw_filler_segments_count': len(preliminary_results['raw_filler_segments']),
...
async def process_preliminary_analysis_async(transcript_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Found {len(aggregated['ad_segments'])} ads, {len(aggregated['filler_segments'])} fillers")
