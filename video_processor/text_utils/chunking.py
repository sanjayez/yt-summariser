...
def chunk_transcript_text(text: str, chunk_size: int = 1000, chunk_overlap: int = 200) -> List[str]:
    """
...
def format_metadata_for_embedding(metadata_obj) -> str:
    ...
    try:
        ...
        if metadata_obj.description:
            ...
            desc = metadata_obj.description[:500] + "..." if len(metadata_obj.description) > 500 else metadata_obj.description
        ...
        if metadata_obj.channel_follower_count:
            parts.append(f"Channel Subscribers: {metadata_obj.channel_follower_count:,}")
        ...
        logger.debug(f"Formatted metadata for video {metadata_obj.video_id}: {len(formatted_text)} characters")
...
def format_summary_for_embedding(summary: str, key_points: List[str]) -> str:
    ...
    try:
        ...
        logger.debug(f"Formatted summary for embedding: {len(formatted_text)} characters")
        return formatted_text
...
def format_segment_for_embedding(segment_obj) -> str:
    ...
    try:
        ...
        video_context = ""
        if hasattr(segment_obj, 'transcript') and hasattr(segment_obj.transcript, 'video_metadata'):
            video_title = segment_obj.transcript.video_metadata.title
            if video_title:
                video_context = f" (from: {video_title[:50]}...)" if len(video_title) > 50 else f" (from: {video_title})"
        
        formatted_text = f"[{timestamp}] {text}{video_context}"
        
        logger.debug(f"Formatted segment {segment_obj.segment_id} for embedding: {len(formatted_text)} characters")
        return formatted_text
...
def prepare_batch_embeddings(items: List[Dict[str, Any]], batch_size: int = 15) -> List[List[Dict[str, Any]]]:
    """
