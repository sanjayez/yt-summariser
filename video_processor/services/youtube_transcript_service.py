...
def extract_youtube_transcript_fallback(
    ...
    try:
        ...
        logger.info(
            f"Found {len(available_transcripts)} available transcripts for {video_id}"
        )
        ...
        for lang in preferred_languages:
            try:
                ...
                used_language = lang
                logger.info(
                    f"Successfully extracted transcript in {lang} for {video_id}"
                )
                break
        ...
        if not transcript_data and available_transcripts:
            for transcript_info in available_transcripts:
                try:
                    ...
                    used_language = lang
                    logger.info(
                        f"Extracted transcript in fallback language {lang} for {video_id}"
                    )
                    break
