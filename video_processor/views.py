...
def transcript_health_check(request):
    ...
    try:
        ...
        successful_transcripts = VideoTranscript.objects.filter(
            status="success"
        ).count()
        transcripts_with_data = (
            VideoTranscript.objects.filter(
                status="success", transcript_text__isnull=False
            )
        ...
        health_status = {
            ...
            "text_coverage": round(
                (transcripts_with_data / successful_transcripts * 100)
                if successful_transcripts > 0
                else 0,
                2,
            ),
            "status": "healthy"
            if transcripts_with_data == successful_transcripts
            else "degraded",
        }
