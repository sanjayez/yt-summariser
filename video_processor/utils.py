    ...
    class TimeoutChecker:
        ...
        def check(self):
            if time.time() - self.start_time > self.timeout_seconds:
                raise TimeoutError(
                    f"{self.op_name} timed out after {self.timeout_seconds} seconds"
                )
...
def update_task_progress(task_instance, step, progress_percent, meta=None):
    ...
    logger.debug(
        f"update_task_progress called for step '{step}' with progress {progress_percent}%"
    )

    meta_data = {"step": step, "progress": progress_percent, **(meta or {})}

    try:
        task_instance.update_state(state="PROGRESS", meta=meta_data)
    except Exception as e:
    ...
    try:
        ...
        if not publish_enabled:
            logger.debug("Video SSE publishing disabled via settings")
            return

        request_uuid = _resolve_request_uuid(task_instance)
        if not request_uuid:
            task_id = getattr(task_instance.request, "id", "unknown")
            getattr(task_instance.request, "args", [])
            getattr(task_instance.request, "kwargs", {})
            logger.debug(
                f"Could not resolve request UUID for task {task_id} - skipping Redis publish"
            )
            return

        message_type, payload = _build_video_progress_payload(
            step, progress_percent, meta or {}
        )
        if not message_type:
        ...
        logger.error(
            f"🔴 REDIS PUBLISH: {message_type} event for video {request_uuid[:8]} to channel {channel} - payload: {payload}"
        )
    except Exception as pub_error:
        # Never fail the task due to progress pub issues
        logger.warning(
            f"Non-fatal: failed to publish video progress update: {pub_error}"
        )


def _resolve_request_uuid(task_instance) -> str | None:
    ...
    try:
        ...
        logger.debug(
            f"No UUID found in task {task_id} - args: {getattr(task_instance.request, 'args', [])}"
        )
        return None
...
def _build_video_progress_payload(
    step: str, progress: int | float, meta: dict
) -> tuple[str | None, dict]:
...
def _publish_to_redis(channel: str, payload: dict) -> None:
    ...
    redis_config = {"host": host, "port": port, "db": 3, "decode_responses": True}
