...
from .models import VideoMetadata, VideoTranscript, TranscriptSegment, VideoExclusionTable, ContentAnalysis
...
class VideoTranscriptAdmin(admin.ModelAdmin):
    list_display = ['video_id', 'video_language', 'status', 'transcript_preview', 'segments_count', 'has_content_analysis', 'created_at']
...
class ContentAnalysisAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Video Info', {
            'fields': ('video_transcript', 'video_link')
        }),
    ...
    def speaker_tones_display(self, obj):
        ...
        return ', '.join(obj.speaker_tones[:3]) + ('...' if len(obj.speaker_tones) > 3 else '')
...
Total Filler Segments: {summary['total_filler_segments']}"""
    ...
    def export_content_analysis_csv(self, request, queryset):
        ...
        writer = csv.writer(response)
        writer.writerow([
            'Video ID', 'Preliminary Status', 'Timestamped Status',
            'Content Rating', 'Ad Ratio', 'Filler Ratio',
            'Speaker Tones', 'Ad Segments Count', 'Filler Segments Count',
            'Created At'
        ])
        
        for analysis in queryset:
            writer.writerow([
                analysis.video_transcript.video_id if analysis.video_transcript else 'N/A',
        ...
        self.message_user(request, f"Exported {queryset.count()} content analysis records.")
        return response
    ...
    def retry_failed_analysis(self, request, queryset):
        ...
        failed_analyses = queryset.filter(
            preliminary_analysis_status='failed'
        ).update(
        ...
        failed_timestamped = queryset.filter(
            ...
            timestamped_analysis_status='pending'
        )
        
        total_reset = failed_analyses + failed_timestamped
        self.message_user(request, f"Reset {total_reset} failed analyses to pending status.")
