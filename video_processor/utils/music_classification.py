...
def classify_music_content(tags: List[str], categories: List[str], title: str, description: str) -> str:
    """
    ...
    try:
        ...
        logger.debug(f"Music classification: music_score={music_score}, background_score={background_score}, "
        ...
        if background_score >= background_threshold:
            logger.info(f"Classified as background music: {background_score} background indicators")
        ...
        # Type 1: Intentional music content (KEEP) - Only if no background indicators
            ...
            logger.info(f"Classified as intentional music: {music_score} music indicators, music category")
            return 'intentional_music'
        
        # Regular content
        else:
            logger.debug(f"Classified as regular content: insufficient music or background indicators")
            return 'not_music'
...
def should_exclude_for_music(video_metadata) -> Tuple[bool, str]:
    ...
    try:
        ...
        if classification == 'background_music':
            logger.info(f"Video {video_metadata.video_id} will be excluded: background music content")
            return True, 'background_music_only'
        else:
            logger.debug(f"Video {video_metadata.video_id} classification: {classification} (not excluded)")
            return False, None
            
    except Exception as e:
        logger.error(f"Error checking music exclusion for video {video_metadata.video_id}: {e}")
        return False, None  # Safe fallback - don't exclude on error


def get_music_classification_summary(tags: List[str], categories: List[str], title: str, description: str) -> dict:
    """
    ...
    try:
        ...
        matching_music_tags = [
            tag for tag in config['primary_music_tags']
            if tag.lower() in all_text
        ]
        
        matching_background_indicators = [
            indicator for indicator in config['background_indicators'] 
            if indicator.lower() in all_text
        ]
