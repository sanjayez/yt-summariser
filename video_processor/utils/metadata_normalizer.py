...
class YouTubeMetadataNormalizer:
    ...
    def normalize_metadata(cls, raw_info: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.debug(
            f"Normalized metadata for video {normalized.get('video_id', 'unknown')}"
        )
        return normalized
    ...
    def _normalize_strings(cls, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(title) > cls.FIELD_CONSTRAINTS["title"]["max_length"]:
            logger.warning(
                f"Title truncated from {len(title)} to {cls.FIELD_CONSTRAINTS['title']['max_length']} chars"
            )
        ...
        if len(channel_name) > cls.FIELD_CONSTRAINTS["channel_name"]["max_length"]:
            logger.warning(
                f"Channel name truncated from {len(channel_name)} to {cls.FIELD_CONSTRAINTS['channel_name']['max_length']} chars"
            )
            channel_name = channel_name[
                : cls.FIELD_CONSTRAINTS["channel_name"]["max_length"]
            ].strip()
        ...
        if len(uploader_id) > cls.FIELD_CONSTRAINTS["uploader_id"]["max_length"]:
            uploader_id = uploader_id[
                : cls.FIELD_CONSTRAINTS["uploader_id"]["max_length"]
            ]
    ...
    def _normalize_numeric_fields(cls, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        if duration is not None:
            try:
                ...
                if duration < cls.FIELD_CONSTRAINTS["duration"]["min_value"]:
                    ...
                    logger.warning(
                        f"Duration {duration} exceeds maximum, capping at {cls.FIELD_CONSTRAINTS['duration']['max_value']}"
                    )
        ...
        if follower_count is not None:
            try:
                ...
                if (
                    follower_count
                    < cls.FIELD_CONSTRAINTS["channel_follower_count"]["min_value"]
                ):
                    follower_count = cls.FIELD_CONSTRAINTS["channel_follower_count"][
                        "default"
                    ]
            except (ValueError, TypeError):
                follower_count = cls.FIELD_CONSTRAINTS["channel_follower_count"][
    ...
    def _normalize_language(
        cls, metadata: dict[str, Any], raw_info: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _normalize_arrays(cls, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        if tags is None:
            ...
            logger.warning(
                f"Tags is not a list: {type(tags)}, converting to empty list"
            )
            tags = []
        else:
            ...
            if len(tags) > cls.FIELD_CONSTRAINTS["tags"]["max_items"]:
                logger.warning(
                    f"Too many tags ({len(tags)}), limiting to {cls.FIELD_CONSTRAINTS['tags']['max_items']}"
                )
        ...
        if categories is None:
            ...
            logger.warning(
                f"Categories is not a list: {type(categories)}, converting to empty list"
            )
            categories = []
        else:
            ...
            if len(categories) > cls.FIELD_CONSTRAINTS["categories"]["max_items"]:
                logger.warning(
                    f"Too many categories ({len(categories)}), limiting to {cls.FIELD_CONSTRAINTS['categories']['max_items']}"
                )
                categories = categories[
                    : cls.FIELD_CONSTRAINTS["categories"]["max_items"]
                ]
        ...
        for field, constraints in cls.FIELD_CONSTRAINTS.items():
            if field not in metadata or metadata[field] is None:
                if "default" in constraints:
                    ...
                    logger.debug(
                        f"Applied default value for field {field}: {constraints['default']}"
                    )
        ...
        for field in [
            ...
            if field in metadata and metadata[field] is not None:
                ...
                if "min_value" in constraints and value < constraints["min_value"]:
                    raise ValueError(
                        f"Field {field} below minimum value {constraints['min_value']}"
                    )

                if "max_value" in constraints and value > constraints["max_value"]:
                    raise ValueError(
                        f"Field {field} above maximum value {constraints['max_value']}"
                    )
    ...
    def _process_heatmap_segments(heatmap_data):
        ...
        if not isinstance(heatmap_data, list):
            logger.warning(
                f"Invalid heatmap data format: expected list, got {type(heatmap_data)}"
            )
            return []
        ...
        try:
            for i, segment in enumerate(heatmap_data):
                ...
                if not isinstance(segment, dict):
                    logger.warning(
                        f"Skipping invalid segment at index {i}: expected dict, got {type(segment)}"
                    )
                    continue
                ...
                if value is None:
                    logger.warning(
                        f"Skipping segment at index {i}: missing 'value' field"
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Skipping segment at index {i}: invalid value '{value}'"
                    )
                    continue
                ...
                if value > 0.95:
                    ...
                    if start_time is None or end_time is None:
                        logger.warning(
                            f"Skipping high engagement segment at index {i}: missing time fields"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping segment at index {i}: invalid time values"
                        )
                        continue
                    ...
                    if start_time < 0 or end_time < 0 or start_time >= end_time:
                        logger.warning(
                            f"Skipping segment at index {i}: invalid time range {start_time}-{end_time}"
                        )
                        continue
            ...
            logger.debug(
                f"Processed {len(heatmap_data)} heatmap segments, found {len(high_engagement_segments)} high engagement segments"
            )
...
def adapt_decodo_to_ytdlp_format(decodo_data: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.debug(
        f"Adapted Decodo metadata for video {yt_dlp_format.get('id')} to yt-dlp format"
    )
