...
def is_video_excluded(video_url: str) -> tuple[bool, str | None]:
    """
...
def filter_excluded_videos(video_urls: list[str]) -> tuple[list[str], list[dict]]:
    ...
    excluded_video_ids = {
        entry["video_id"]: entry["exclusion_reason"] for entry in excluded_entries
    }
    ...
    for video_id, url in video_id_to_url.items():
        if video_id in excluded_video_ids:
            ...
            logger.info(
                f"Pre-filtered excluded video {video_id}: {excluded_video_ids[video_id]}"
            )
        else:
            processable_urls.append(url)

    logger.info(
        f"Pre-filtering results: {len(processable_urls)} processable, {len(excluded_info)} excluded"
    )
    return processable_urls, excluded_info


def add_video_to_exclusion_table(video_url: str, exclusion_reason: str) -> bool:
    """
    ...
    if exclusion_reason not in valid_reasons:
        logger.error(
            f"Invalid exclusion reason: {exclusion_reason}. Valid options: {valid_reasons}"
        )
        return False
...
def get_exclusion_statistics() -> dict:
    ...
    try:
        ...
        reason_stats = (
            VideoExclusionTable.objects.values("exclusion_reason")
            .annotate(count=Count("exclusion_reason"))
            .order_by("-count")
        )
        ...
        return {
            ...
            "top_exclusion_reason": reason_stats[0]["exclusion_reason"]
            if reason_stats
            else None,
        }
    except Exception as e:
