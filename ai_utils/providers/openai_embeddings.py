...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    async def embed_text(self, text_or_request: Union[str, EmbeddingRequest]) -> EmbeddingResponse:
    ...
    async def embed_batch(self, batch_request: BatchEmbeddingRequest) -> BatchEmbeddingResponse:
        ...
        try:
            ...
            for i in range(0, len(texts), self.batch_size):
                ...
                # Accumulate usage
                total_usage["prompt_tokens"] += chunk_response.usage.get("prompt_tokens", 0)
                total_usage["total_tokens"] += chunk_response.usage.get("total_tokens", 0)
    ...
    async def _embed_batch_direct(self, texts: List[str], model: str, user: Optional[str]) -> BatchEmbeddingResponse:
    ...
    async def estimate_cost(self, text_count: int, model: Optional[str] = None) -> Dict[str, Any]:
        """Estimate cost for embedding operations"""
        model = model or self.config.openai.embedding_model
        
        # Rough cost estimates (tokens per 1000 characters, cost per 1M tokens)
        model_costs = {
            "text-embedding-3-small": {"tokens_per_1k_chars": 300, "cost_per_1m_tokens": 0.02},
            "text-embedding-3-large": {"tokens_per_1k_chars": 300, "cost_per_1m_tokens": 0.13},
            "text-embedding-ada-002": {"tokens_per_1k_chars": 400, "cost_per_1m_tokens": 0.10}
        }
