...
class WeaviateVectorStoreProvider(VectorStoreProvider):
                ...
                try:
                    ...
                    self.client.collections.create(
                        ...
                        properties=[
                            ...
                            wvc.config.Property(
                                name="original_id", data_type=wvc.config.DataType.TEXT
                            ),
                            ...
                            wvc.config.Property(
                                name="video_id", data_type=wvc.config.DataType.TEXT
                            ),
                            wvc.config.Property(
                                name="title", data_type=wvc.config.DataType.TEXT
                            ),
                            wvc.config.Property(
                                name="channel", data_type=wvc.config.DataType.TEXT
                            ),
                            wvc.config.Property(
                                name="duration", data_type=wvc.config.DataType.NUMBER
                            ),
                            wvc.config.Property(
                                name="chunk_index", data_type=wvc.config.DataType.NUMBER
                            ),
                            wvc.config.Property(
                                name="start_time", data_type=wvc.config.DataType.NUMBER
                            ),
                            wvc.config.Property(
                                name="end_time", data_type=wvc.config.DataType.NUMBER
                            ),
                            wvc.config.Property(
                            ...
                            wvc.config.Property(
                                name="youtube_url", data_type=wvc.config.DataType.TEXT
                            ),
                            wvc.config.Property(
                    ...
                    logger.warning(
                        f"Failed to create collection, but continuing: {create_error}"
                    )
            else:
        ...
        try:
            ...
            for i in range(0, len(objects), batch_size):
                ...
                successful_inserts = len(
                    [obj for obj in response.uuids.values() if obj is not None]
                )
        ...
        try:
            ...
            if where_filter:
                response = self.collection.query.near_text(
                    query=query.query, filters=where_filter, **search_kwargs
                )
            else:
                response = self.collection.query.near_text(
                    query=query.query, **search_kwargs
                )
            ...
            for obj in response.objects:
                ...
                result = VectorSearchResult(
                    id=obj.properties.get(
                        "original_id", str(obj.uuid)
    ...
    def _build_where_filter(self, filters: dict[str, Any]) -> wvc.query.Filter | None:
        ...
        try:
            ...
            for key, value in filters.items():
                if isinstance(value, str):
                    ...
                    if "gte" in value:
                        conditions.append(
                            wvc.query.Filter.by_property(key).greater_or_equal(
                                value["gte"]
                            )
                        )
                    if "lte" in value:
                        conditions.append(
                            wvc.query.Filter.by_property(key).less_or_equal(
                                value["lte"]
                            )
                        )
                    if "eq" in value:
                        conditions.append(
                            wvc.query.Filter.by_property(key).equal(value["eq"])
                        )
            ...
            for i in range(0, len(document_ids), batch_size):
                ...
                try:
                    ...
                    successful_deletes = len(
                        batch_ids
                    )  # Assume all successful unless error
                    ...
                    for doc_id in batch_ids:
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete document {doc_id}: {str(e)}"
                            )
    ...
    async def get_document(self, document_id: str) -> VectorDocument | None:
        ...
        try:
            ...
            obj = self.collection.query.fetch_object_by_id(uuid=document_id)
        ...
        try:
            ...
            collections.create(
                ...
                properties=[
                    ...
                    wvc.config.Property(
                        name="sequence_number", data_type=wvc.config.DataType.NUMBER
                    ),
                    wvc.config.Property(
                        name="youtube_url", data_type=wvc.config.DataType.TEXT
                    ),
                    wvc.config.Property(
                        name="key_points_count", data_type=wvc.config.DataType.NUMBER
                    ),
                ],
                vectorizer_config=wvc.config.Configure.Vectorizer.text2vec_weaviate(),
                vector_index_config=wvc.config.Configure.VectorIndex.hnsw(
    ...
    async def list_indices(self) -> list[str]:
        ...
        try:
            ...
            collection_names = [
                class_def["class"] for class_def in schema.get("classes", [])
            ]
        ...
        try:
            ...
            for class_def in schema.get("classes", []):
                ...
                config = IndexConfig(
                    ...
                    dimension=vector_config.get(
                        "vectorCacheMaxObjects", self.config.weaviate.dimension
                    ),
