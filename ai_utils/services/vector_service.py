...
class VectorService:
    ...
    async def upsert_documents(
        ...
        try:
            ...
            logger.info(
                f"Upserted {job.processed_items} documents in {processing_time:.2f}ms"
            )

            # Return a dictionary with the result data
            return {
                "upserted_count": result.processed_items
                if hasattr(result, "processed_items")
                else len(documents),
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.error(
                f"Failed to upsert documents in {processing_time:.2f}ms: {str(e)}"
            )
            raise

    async def search_similar(
        self, query: VectorQuery, index_name: str = None
    ) -> VectorSearchResponse:
        """
        ...
        try:
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.info(
                f"Search completed in {processing_time:.2f}ms, found {len(response.results)} results"
            )
    ...
    async def delete_documents(
        self, document_ids: list[str], index_name: str = None, job_id: str = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.info(
                f"Deleted {job.processed_items} documents in {processing_time:.2f}ms"
            )
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.error(
                f"Failed to delete documents in {processing_time:.2f}ms: {str(e)}"
            )
            raise

    async def get_document(
        self, document_id: str, index_name: str = None
    ) -> VectorDocument | None:
        """
    ...
    async def bulk_upsert_with_embedding(
        ...
        try:
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.info(
                f"Bulk upsert with native embedding completed in {processing_time:.2f}ms"
            )
            ...
            processing_time = (time.time() - start_time) * 1000
            logger.error(
                f"Failed to bulk upsert with native embedding in {processing_time:.2f}ms: {str(e)}"
            )
            raise
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "supported_metrics": self.provider.get_supported_metrics()
            if self.provider
            else [],
        }
