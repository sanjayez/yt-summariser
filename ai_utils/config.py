...
class OpenAIConfig(BaseModel):
    ...
    chat_model: str = Field(
        default="gpt-3.5-turbo", description="Default chat completion model"
    )
    temperature: float = Field(
        default=0.7, ge=0.0, le=2.0, description="Default generation temperature"
    )
    max_tokens: int | None = Field(
        None, ge=1, description="Default max tokens for generation"
    )
    top_p: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Default top-p sampling"
    )
    frequency_penalty: float = Field(
        default=0.0, ge=-2.0, le=2.0, description="Default frequency penalty"
    )
    presence_penalty: float = Field(
        default=0.0, ge=-2.0, le=2.0, description="Default presence penalty"
    )

    # API settings
    base_url: str | None = Field(
        None, description="OpenAI API base URL (for custom endpoints)"
    )
    timeout: int = Field(default=30, description="Request timeout in seconds")
    max_retries: int = Field(default=3, description="Maximum retry attempts")
    user: str | None = Field(None, description="User identifier for API requests")
...
class GeminiConfig(BaseModel):
    ...
    max_tokens: int | None = Field(
        default=4000, ge=1, description="Default max tokens for generation"
    )
    top_p: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Default top-p sampling"
    )
    top_k: int | None = Field(default=40, ge=1, description="Default top-k sampling")
...
class PineconeConfig(BaseModel):
    ...
    environment: str = Field(
        ..., description="Pinecone environment (e.g., 'aws-starter')"
    )
    index_name: str = Field(
        default="youtube-transcripts", description="Default index name"
    )
    dimension: int = Field(
...
class WeaviateConfig(BaseModel):
    ...
    api_key: str | None = Field(
        None, description="Weaviate API key (for Weaviate Cloud)"
    )
    collection_name: str = Field(
        default="YoutubeTranscripts", description="Default collection name"
    )
    dimension: int = Field(
        default=768, description="Vector dimension (for Arctic-Embed-m-v2.0)"
    )
    ...
    cluster_url: str | None = Field(None, description="WCS cluster URL (overrides url)")
    auth_client_secret: str | None = Field(None, description="WCS auth client secret")
...
class LlamaIndexConfig(BaseModel):
    ...
    similarity_top_k: int = Field(
        default=5, description="Number of similar documents to retrieve"
    )
    response_mode: str = Field(
        default="compact", description="Response generation mode"
    )
        ...
        return cls(
            ...
            weaviate=WeaviateConfig(
                ...
                collection_name=os.getenv(
                    "WEAVIATE_COLLECTION_NAME", "YoutubeTranscripts"
                ),
