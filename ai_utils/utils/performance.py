...
from ..models import EmbeddingResponse, BatchEmbeddingResponse
...
class PerformanceBenchmark:
    ...
    async def benchmark_embedding_operation(
        ...
        if times:
            ...
            avg_time = min_time = max_time = median_time = std_dev = items_per_second = 0
    ...
    async def benchmark_batch_operation(
        ...
        # Actual benchmark runs
        logger.info(f"Running benchmark for {operation_name} with {len(test_data)} batches")
        for i, batch in enumerate(test_data):
            try:
                ...
                logger.debug(f"Processed batch {i + 1}/{len(test_data)} ({len(batch)} items)")
...
async def benchmark_embedding_service(
    ...
    await benchmark.benchmark_embedding_operation(
        ...
        metadata={"model": getattr(service.config, 'openai', {}).get('model', 'unknown')}
    )
