...
class SearchProgressRequest(BaseModel):
    ...
    max_videos: int = Field(default=5, ge=1, le=20, description="Maximum number of videos to process")
    priority: ProcessingPriority = Field(default=ProcessingPriority.NORMAL, description="Processing priority")
    enable_parallel: bool = Field(default=True, description="Enable parallel video processing")
    max_parallel_workers: int = Field(default=3, ge=1, le=10, description="Maximum parallel workers")
    timeout_seconds: int = Field(default=1800, ge=60, le=3600, description="Overall timeout in seconds")
    
    # Processing configuration
    include_metadata: bool = Field(default=True, description="Include video metadata processing")
    include_transcript: bool = Field(default=True, description="Include transcript processing")
    include_embedding: bool = Field(default=True, description="Include embedding processing")
    include_summary: bool = Field(default=True, description="Include summary generation")
    
    # Search configuration
    search_filters: Optional[Dict[str, Any]] = Field(default=None, description="Search filters")
...
class SearchProgressResponse(BaseModel):
    ...
    successful_videos: int = Field(default=0, description="Number of successfully processed videos")
    failed_videos: int = Field(default=0, description="Number of failed videos")
    
    # Timing information
    search_start_time: Optional[datetime] = Field(default=None, description="Search start timestamp")
    search_end_time: Optional[datetime] = Field(default=None, description="Search end timestamp")
    processing_start_time: Optional[datetime] = Field(default=None, description="Processing start timestamp")
    processing_end_time: Optional[datetime] = Field(default=None, description="Processing end timestamp")
    
    # Results
    video_urls: List[str] = Field(default_factory=list, description="List of video URLs")
    processing_results: List["VideoProcessingResult"] = Field(default_factory=list, description="Video processing results")
...
class VideoProcessingRequest(BaseModel):
    ...
    # Processing configuration
    include_metadata: bool = Field(default=True, description="Include metadata processing")
    include_transcript: bool = Field(default=True, description="Include transcript processing")
    include_embedding: bool = Field(default=True, description="Include embedding processing")
    include_summary: bool = Field(default=True, description="Include summary generation")
    ...
    retry_delay: int = Field(default=60, ge=0, le=300, description="Retry delay in seconds")
...
class VideoProcessingResult(BaseModel):
    ...
    # Processing stages
    metadata_status: VideoProcessingStatus = Field(default=VideoProcessingStatus.PENDING, description="Metadata processing status")
    transcript_status: VideoProcessingStatus = Field(default=VideoProcessingStatus.PENDING, description="Transcript processing status")
    embedding_status: VideoProcessingStatus = Field(default=VideoProcessingStatus.PENDING, description="Embedding processing status")
    summary_status: VideoProcessingStatus = Field(default=VideoProcessingStatus.PENDING, description="Summary processing status")
    
    # Results
    metadata_available: bool = Field(default=False, description="Whether metadata is available")
    transcript_available: bool = Field(default=False, description="Whether transcript is available")
    embeddings_available: bool = Field(default=False, description="Whether embeddings are available")
    summary_available: bool = Field(default=False, description="Whether summary is available")
...
class VideoProcessingStatusAggregation(BaseModel):
    ...
    # Processing stages
    metadata_completed: int = Field(default=0, description="Videos with metadata completed")
    transcript_completed: int = Field(default=0, description="Videos with transcript completed")
    embedding_completed: int = Field(default=0, description="Videos with embeddings completed")
    summary_completed: int = Field(default=0, description="Videos with summary completed")
    ...
    # Performance metrics
    average_processing_time: Optional[float] = Field(default=None, description="Average processing time per video")
    throughput_videos_per_minute: Optional[float] = Field(default=None, description="Processing throughput")
...
class ParallelProcessingConfig(BaseModel):
    ...
    max_workers: int = Field(default=3, ge=1, le=10, description="Maximum parallel workers")
    chunk_size: int = Field(default=5, ge=1, le=20, description="Number of videos per chunk")
    enable_load_balancing: bool = Field(default=True, description="Enable load balancing")
    worker_timeout: int = Field(default=900, ge=60, le=1800, description="Worker timeout in seconds")
    
    # Resource limits
    max_memory_mb: int = Field(default=1024, ge=512, le=4096, description="Maximum memory per worker")
    max_cpu_percent: int = Field(default=80, ge=10, le=100, description="Maximum CPU usage percent")
...
class WorkerStatus(BaseModel):
    ...
    current_task: Optional[str] = Field(default=None, description="Current task being processed")
...
class ParallelProcessingOrchestration(BaseModel):
    ...
    config: ParallelProcessingConfig = Field(..., description="Processing configuration")
    
    # Worker management
    workers: List[WorkerStatus] = Field(default_factory=list, description="Worker status list")
    ...
    # Performance metrics
    throughput_tasks_per_minute: Optional[float] = Field(default=None, description="Task throughput")
    average_task_time: Optional[float] = Field(default=None, description="Average task processing time")
...
class ProcessingError(BaseModel):
    ...
    request_id: str = Field(..., description="Request identifier")
    video_id: Optional[str] = Field(default=None, description="Video ID if applicable")
    ...
    # Context information
    processing_stage: str = Field(..., description="Processing stage where error occurred")
    worker_id: Optional[str] = Field(default=None, description="Worker ID if applicable")
    ...
    # Metadata
    occurred_at: datetime = Field(default_factory=datetime.now, description="Error occurrence timestamp")
    resolved_at: Optional[datetime] = Field(default=None, description="Error resolution timestamp")

class RetryConfig(BaseModel):
    """Configuration for retry logic"""
    max_retries: int = Field(default=3, ge=0, le=10, description="Maximum retry attempts")
    initial_delay: int = Field(default=60, ge=0, le=300, description="Initial retry delay in seconds")
    max_delay: int = Field(default=600, ge=60, le=3600, description="Maximum retry delay in seconds")
    backoff_factor: float = Field(default=2.0, ge=1.0, le=10.0, description="Exponential backoff factor")
...
class RetryTask(BaseModel):
    ...
    # Original error
    original_error: ProcessingError = Field(..., description="Original error that triggered retry")
...
class SearchToProcessRequest(BaseModel):
    ...
    # Search configuration
    max_videos: int = Field(default=5, ge=1, le=20, description="Maximum videos to process")
    search_filters: Optional[Dict[str, Any]] = Field(default=None, description="Search filters")
    
    # Processing configuration
    processing_config: VideoProcessingRequest = Field(..., description="Video processing configuration")
    parallel_config: ParallelProcessingConfig = Field(default_factory=ParallelProcessingConfig, description="Parallel processing configuration")
    retry_config: RetryConfig = Field(default_factory=RetryConfig, description="Retry configuration")
    
    # Priority and timing
    priority: ProcessingPriority = Field(default=ProcessingPriority.NORMAL, description="Processing priority")
    timeout_seconds: int = Field(default=1800, ge=60, le=3600, description="Overall timeout")
...
class SearchToProcessResponse(BaseModel):
    ...
    video_statuses: List[VideoProcessingResult] = Field(default_factory=list, description="Video processing results")
    aggregated_status: VideoProcessingStatusAggregation = Field(..., description="Aggregated status")
    
    # Parallel processing information
    orchestration: Optional[ParallelProcessingOrchestration] = Field(default=None, description="Parallel processing orchestration")
    
    # Error handling
    errors: List[ProcessingError] = Field(default_factory=list, description="Processing errors")
    retry_tasks: List[RetryTask] = Field(default_factory=list, description="Retry tasks")
    
    # Performance metrics
    total_processing_time: Optional[float] = Field(default=None, description="Total processing time in seconds")
    videos_per_minute: Optional[float] = Field(default=None, description="Processing throughput")
...
class StatusCheckResponse(BaseModel):
    ...
    details: Optional[SearchToProcessResponse] = Field(default=None, description="Detailed status information")
    
    # Estimated completion
    estimated_completion: Optional[datetime] = Field(default=None, description="Estimated completion time")
    
    # Response metadata
    checked_at: datetime = Field(default_factory=datetime.now, description="Status check timestamp")
