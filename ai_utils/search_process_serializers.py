...
class SearchProgressRequestSerializer(serializers.Serializer):
    ...
    max_parallel_workers = serializers.IntegerField(
        min_value=1, max_value=10, default=3
    )
    timeout_seconds = serializers.IntegerField(
        min_value=60, max_value=3600, default=1800
    )
...
class VideoProcessingResultSerializer(serializers.Serializer):
    ...
    # Processing stages
    metadata_status = VideoProcessingStatusSerializer(
        default=VideoProcessingStatus.PENDING.value
    )
    transcript_status = VideoProcessingStatusSerializer(
        default=VideoProcessingStatus.PENDING.value
    )
    embedding_status = VideoProcessingStatusSerializer(
        default=VideoProcessingStatus.PENDING.value
    )
    summary_status = VideoProcessingStatusSerializer(
        default=VideoProcessingStatus.PENDING.value
    )
...
class VideoProcessingStatusAggregationSerializer(serializers.Serializer):
    ...
    throughput_videos_per_minute = serializers.FloatField(
        allow_null=True, required=False
    )
...
class ParallelProcessingConfigSerializer(serializers.Serializer):
    ...
    # Resource limits
    max_memory_mb = serializers.IntegerField(
        min_value=512, max_value=4096, default=1024
    )
...
class ParallelProcessingOrchestrationSerializer(serializers.Serializer):
    ...
    # Performance metrics
    throughput_tasks_per_minute = serializers.FloatField(
        allow_null=True, required=False
    )
...
class RetryConfigSerializer(serializers.Serializer):
    ...
    retryable_errors = serializers.ListField(
        ...
        default=lambda: [
            ...
            ErrorType.PROCESSING_TIMEOUT.value,
            ErrorType.QUOTA_EXCEEDED.value,
        ],
    )
...
class SearchToProcessRequestSerializer(serializers.Serializer):
    ...
    def to_pydantic(self) -> SearchToProcessRequest:
        ...
        if "processing_config" in validated_data:
            processing_serializer = VideoProcessingRequestSerializer(
                data=validated_data["processing_config"]
            )
            if processing_serializer.is_valid():
                validated_data["processing_config"] = (
                    processing_serializer.to_pydantic()
                )

        if "parallel_config" in validated_data:
            parallel_serializer = ParallelProcessingConfigSerializer(
                data=validated_data["parallel_config"]
            )
        ...
        if "retry_config" in validated_data:
            retry_serializer = RetryConfigSerializer(
                data=validated_data["retry_config"]
            )
...
class SearchToProcessResponseSerializer(serializers.Serializer):
    ...
    orchestration = ParallelProcessingOrchestrationSerializer(
        allow_null=True, required=False
    )
