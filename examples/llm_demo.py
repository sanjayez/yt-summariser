...
from ai_utils.models import (
    ChatMessage, ChatRole, TextGenerationRequest, RAGQuery, VectorSearchResult
)
...
async def main():
    ...
    try:
        ...
        if llm_health['status'] != 'healthy':
            logger.error("❌ LLM service is unhealthy. Please check your OpenAI API configuration.")
            return
        
        logger.info(f"Supported models: {llm_health['supported_models'][:3]}...")  # Show first 3
        ...
        detailed_result = await llm_service.generate_text_with_response(detailed_request)
        ...
        logger.info(f"Usage: {detailed_result['response'].usage.prompt_tokens} prompt + {detailed_result['response'].usage.completion_tokens} completion = {detailed_result['response'].usage.total_tokens} total tokens")
        logger.info(f"Generation time: {detailed_result['response'].generation_time_ms:.1f}ms")
        ...
        rag_result = await llm_service.generate_rag_response(
            query=rag_query,
            context_documents=mock_context_documents
        )
        ...
        conversation_messages = [
            ChatMessage(role=ChatRole.SYSTEM, content="You are a helpful AI assistant discussing machine learning concepts."),
            ChatMessage(role=ChatRole.USER, content="What is overfitting in machine learning?"),
        ]
        ...
        conversation_messages.append(ChatMessage(role=ChatRole.ASSISTANT, content=first_response))
        conversation_messages.append(ChatMessage(role=ChatRole.USER, content="How can I prevent overfitting?"))
        ...
        logger.info(f"User: What is overfitting in machine learning?")
        logger.info(f"Assistant: {first_response}")
        logger.info(f"User: How can I prevent overfitting?")
        ...
        if stats['benchmark_stats']:
            for operation, metrics in stats['benchmark_stats'].items():
                logger.info(f"{operation}: avg {metrics['avg_ms']:.1f}ms (count: {metrics['count']})")
        
        logger.info("\n🎉 LLM Demo completed successfully!")
        logger.info("=" * 65)
        logger.info("✅ The AI Utils LLM interaction layer is fully functional!")
        logger.info("🔧 You can now use both vector store and LLM capabilities together")
