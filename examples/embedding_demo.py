...
from ai_utils.utils.performance import benchmark_embedding_service
...
async def demo_batch_embedding():
    ...
    try:
        ...
        print(
            f"   Average tokens per text: {response.usage['prompt_tokens'] / len(texts):.1f}"
        )
...
async def demo_performance_benchmarking():
    ...
    try:
        benchmark = await benchmark_embedding_service(
            service=service, test_texts=test_texts, batch_sizes=[1, 5, 10]
        )
...
async def demo_health_check():
    ...
    try:
        ...
        print(
            f"Provider health: {'✅ Healthy' if provider_healthy else '❌ Unhealthy'}"
        )
