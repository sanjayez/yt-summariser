...
async def main():
    ...
    try:
        ...
        if health_result['status'] != 'healthy':
            logger.error("❌ QueryProcessor service is unhealthy. Please check your configuration.")
            return
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            if result["status"] == "completed":
                ...
                logger.error(f"{i}. Failed to enhance: '{query}' - {result.get('error', 'Unknown error')}")
            logger.info("")
        ...
        if batch_result["status"] == "completed":
            ...
            for result in batch_result['results']:
                if result['status'] == 'completed':
                    logger.info(f"   '{result['original_query']}' → '{result['enhanced_query']}'")
                else:
        ...
        for i, query in enumerate(edge_cases, 1):
            ...
            if result["status"] == "completed":
                ...
                logger.info(f"   Input: '{query[:50]}{'...' if len(query) > 50 else ''}'")
                logger.info(f"   Output: '{result['enhanced_query']}'")
            else:
                logger.info(f"{i}. Expected error for edge case: {result.get('error', 'Unknown error')}")
            logger.info("")
        ...
        tasks = [
            query_processor.enhance_query(query) 
        ...
        logger.info(f"   Average per query: {concurrent_time/len(performance_queries):.1f}ms")
        
        successful_concurrent = sum(1 for r in concurrent_results if r['status'] == 'completed')
        logger.info(f"   Success rate: {successful_concurrent}/{len(performance_queries)} ({successful_concurrent/len(performance_queries)*100:.1f}%)")
