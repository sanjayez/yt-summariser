    ...
    for query, target_results in test_queries:
        ...
        try:
            ...
            if len(results) == target_results:
                ...
                print(
                    f"⚠️  PARTIAL: Found {len(results)}/{target_results} (may have hit safety limit)"
                )
            else:
                print(
                    f"❌ ERROR: Got more results than requested ({len(results)} > {target_results})"
                )
...
def test_filtering_effectiveness():
    ...
    try:
        ...
        unfiltered_results = unfiltered_provider.search_with_metadata(
            query, max_results=3
        )
        ...
        if filtered_results:
            ...
            print("\n📊 Analysis:")
            print(
                f"   - All filtered results have duration ≥ 60s: {all(d and ':' in d for d in durations)}"
            )
