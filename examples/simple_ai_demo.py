...
from ai_utils.models import ChatMessage, ChatRole, TextGenerationRequest
...
async def demo_conversation_flow(llm_service):
    ...
    for i, user_input in enumerate(user_inputs, 1):
        try:
            ...
            conversation.append(ChatMessage(role=ChatRole.ASSISTANT, content=assistant_response))
...
async def demo_batch_processing(llm_service):
    ...
    try:
        ...
        logger.info(f"✅ Generated {result['total_processed']} definitions in {result['processing_time_ms']:.1f}ms:")
...
async def main():
    ...
    try:
        ...
        logger.info(f"✅ LLM service is healthy!")
