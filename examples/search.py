...
def demo_scrapetube_provider():
    ...
    provider = ScrapeTubeProvider(
        max_results=3, filter_shorts=True, english_only=True, min_duration_seconds=60
    )
...
def demo_filtering_comparison():
    ...
    provider_unfiltered = ScrapeTubeProvider(
        max_results=5, filter_shorts=False, english_only=False
    )
    ...
    try:
        ...
        unfiltered_results = provider_unfiltered.search_with_metadata(
        ...
        filtered_results = provider_filtered.search_with_metadata(
            search_query, max_results=3
        )
...
def demo_youtube_search_service():
    ...
    try:
        request = SearchRequest(
            query="machine learning basics", max_results=2, include_metadata=True
        )
...
def demo_iterative_fetching():
    ...
    search_query = (
        "tutorial"  # Broad query that will have many results but many filtered out
    )

    print(f"Searching for: '{search_query}'")
    print("Target: 5 English videos longer than 3 minutes")
    print(
        "This will demonstrate iterative fetching until we find enough filtered results..."
    )
    ...
    try:
        ...
        logging.getLogger("topic.services.providers.scrapetube_provider").setLevel(
            logging.INFO
        )
        ...
        if len(results) < 5:
            print(
                f"⚠️  Note: Only found {len(results)}/5 results - may have hit the 100 video safety limit"
            )
