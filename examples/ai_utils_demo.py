...
async def main():
    ...
    try:
        ...
        if not all(
            h["status"] == "healthy"
            for h in [embedding_health, vector_health, llm_health]
        ):
            logger.error(
                "❌ One or more services are unhealthy. Please check your configuration."
            )
            return
        ...
        text_result = await llm_service.generate_text(
            prompt=simple_prompt, temperature=0.7, max_tokens=150
        )
        ...
        logger.info(
            f"Chat response: {chat_result['response'].choices[0].message.content}"
        )
        ...
        logger.info(
            f"✅ Bulk upserted {bulk_result['upserted_count']} documents in {bulk_result['processing_time_ms']:.2f}ms"
        )
        ...
        logger.info(
            f"Found {len(search_result['results'])} results in {search_result['processing_time_ms']:.2f}ms:"
        )
        ...
        rag_result = await llm_service.generate_rag_response(
            query=rag_query, context_documents=context_documents
        )
        ...
        logger.info(
            f"✅ Batch generated {batch_result['total_processed']} responses in {batch_result['processing_time_ms']:.2f}ms"
        )
        ...
        logger.info(
            f"Embedding service jobs: {embedding_stats['total_jobs']} (completed: {embedding_stats['completed_jobs']})"
        )
        logger.info(
            f"Vector service jobs: {vector_stats['total_jobs']} (completed: {vector_stats['completed_jobs']})"
        )
        logger.info(
            f"LLM service jobs: {llm_stats['total_jobs']} (completed: {llm_stats['completed_jobs']})"
        )
        ...
        filtered_embedding = await embedding_service.embed_text(
        ...
        filtered_rag_result = await llm_service.generate_rag_response(
            query=advanced_rag_query, context_documents=filtered_context
        )
