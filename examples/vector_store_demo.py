...
from ai_utils.providers import OpenAIEmbeddingProvider, WeaviateVectorStoreProvider
from ai_utils.services import EmbeddingService, VectorService
...
async def demonstrate_vector_store_operations():
    ...
    logger.info(
        f"Embedding service health: {'✅ Healthy' if embedding_health else '❌ Unhealthy'}"
    )
    logger.info(
        f"Vector service health: {'✅ Healthy' if vector_health else '❌ Unhealthy'}"
    )
    ...
    try:
        ...
        for index in indexes:
            logger.info(
                f"  - {index.name} (dimension: {index.dimension}, metric: {index.metric})"
            )
    ...
    try:
        ...
        logger.info(
            f"Index stats: {stats.total_vector_count} vectors, {stats.dimension} dimensions, {stats.index_fullness:.2%} fullness"
        )
    except Exception as e:
    ...
    logger.info(
        f"✅ Upserted {result['upserted_count']} documents in {benchmark.elapsed_time:.2f}ms"
    )
    ...
    for query in search_queries:
        ...
        logger.info(
            f"Found {len(response.results)} results in {benchmark.elapsed_time:.2f}ms:"
        )
    ...
    logger.info(
        f"Found {len(ai_response.results)} AI-related documents in {benchmark.elapsed_time:.2f}ms:"
    )
    ...
    logger.info(
        f"✅ Bulk upserted {bulk_result['upserted_count']} documents in {benchmark.elapsed_time:.2f}ms"
    )
    ...
    try:
        ...
        for index in indexes:
            ...
            try:
                ...
                logger.info(
                    f"    Stats: {stats.total_vector_count} vectors, {stats.index_fullness:.2%} fullness"
                )
            except Exception as e:
...
async def main():
    ...
    try:
        ...
        config = get_config()
        logger.info(
            f"Configuration loaded: Weaviate index '{config.weaviate.index_name}'"
        )
        ...
        if existing_indexes:
            ...
            logger.info(
                f"Found existing index: {existing_index.name} (dimension: {existing_index.dimension})"
            )
