...
class TopicQueryService:
    ...
    async def process_topic_query(self, user_query: str, topic_context: str = None) -> dict:
        """
        ...
        try:
            ...
            if topic_context:
                contextual_query = f"{enhanced_result['enhanced_query']} {topic_context}"
                
                # Re-enhance with context
                final_result = await self.query_processor.enhance_query(contextual_query)
                ...
                if final_result["status"] == "completed":
                    return {
                        ...
                        "status": "completed",
                        "processing_time_ms": enhanced_result["processing_time_ms"] + final_result["processing_time_ms"],
                        "tokens_used": enhanced_result["tokens_used"] + final_result["tokens_used"]
                    }
                else:
    ...
    try:
        ...
        for i, (query, topic) in enumerate(test_cases, 1):
            ...
            if result["status"] == "completed":
                ...
                logger.info(f"   Time: {result['processing_time_ms']:.1f}ms | Tokens: {result['tokens_used']}")
                
                if result.get("context_enhancement_failed"):
                    logger.warning(f"   ⚠️  Context enhancement failed, using basic enhancement")
            else:
        ...
        if batch_result["status"] == "completed":
            ...
            logger.info(f"   Total time: {batch_result['total_processing_time_ms']:.1f}ms")
            logger.info(f"   Total tokens: {batch_result['total_tokens_used']}")
            logger.info("")
            
            for result in batch_result['results']:
                if result['status'] == 'completed':
                    logger.info(f"   '{result['original_query']}' + '{result['topic_context']}'")
                    logger.info(f"   → '{result['contextual_query']}'")
                else:
                    logger.error(f"   Failed: '{result['original_query']}' - {result.get('error', 'Unknown error')}")
        else:
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            if result["status"] == "completed":
                ...
                logger.info(f"   Performance: {result['processing_time_ms']:.1f}ms, {result['tokens_used']} tokens")
            else:
        ...
        # With context
        contextual_result = await topic_service.process_topic_query(test_query, "web security")
        
        if basic_result["status"] == "completed" and contextual_result["status"] == "completed":
            ...
            time_overhead = contextual_result['processing_time_ms'] - basic_result['processing_time_ms']
            token_overhead = contextual_result['tokens_used'] - basic_result['tokens_used']
            
            logger.info(f"Context overhead: +{time_overhead:.1f}ms, +{token_overhead} tokens")
