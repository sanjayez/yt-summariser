...
def handle_api_errors(service_name: str, raise_on_error: bool = True):
    ...
    try:
        ...
        error_details = {"service": service_name, "original_error": type(e).__name__}
        ...
        # Log the error
        logger.error(
            f"{service_name} error: {str(e)}", extra=error_details, exc_info=True
        )

        if raise_on_error:
            # Convert to ExternalServiceError
            raise ExternalServiceError(
                service=service_name, message=str(e), **error_details
            ) from e


def handle_timeout(timeout_seconds: float, error_message: str | None = None):
    """
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
                try:
                    ...
                    raise VideoProcessingError(
                        msg, step=func.__name__, timeout_seconds=timeout_seconds
                    ) from e
...
__all__ = ["handle_exceptions", "handle_api_errors", "handle_timeout"]
