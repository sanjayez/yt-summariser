...
async def safe_cleanup(
    cleanup_func: Callable[[], Any], context: str = "cleanup"
) -> None:
...
def with_error_context(
    context_name: str,
    context_value_param: str | int | None = None,
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if context_value_param is not None:
                if (
                    isinstance(context_value_param, int)
                    and len(args) > context_value_param
                ):
                    context_value = args[context_value_param]
                elif (
                    isinstance(context_value_param, str)
                    and context_value_param in kwargs
                ):
