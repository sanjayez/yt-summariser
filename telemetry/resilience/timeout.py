    ...
    **kwargs: Any,
    ...
    try:
        if asyncio.iscoroutine(coro_or_func):
            ...
            return await asyncio.wait_for(
                coro_or_func(*args, **kwargs), timeout=timeout_seconds
            )
    except builtins.TimeoutError as e:
        operation_name = getattr(coro_or_func, "__name__", str(coro_or_func))
        msg = (
            error_message
            or f"Operation '{operation_name}' timed out after {timeout_seconds}s"
        )
        logger.error(
            msg,
            extra={
        ...
        raise TimeoutError(
            operation_name, timeout_seconds, args=args, kwargs=kwargs
        ) from e
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if execution_time > timeout_seconds:
            msg = (
                error_message
                or f"{func.__name__} took {execution_time:.2f}s (exceeded expected timeout of {timeout_seconds}s)"
            )
            logger.warning(
                msg,
                extra={
...
class TimeoutContext:
    ...
    async def __aenter__(self):
        ...
        logger.debug(
            f"Starting {self.operation_name} with {self.timeout_seconds}s timeout"
        )
        return self
        ...
        if self.start_time is not None:
            ...
            if exc_type is None:
                if execution_time > self.timeout_seconds:
                    logger.warning(
                        f"{self.operation_name} took {execution_time:.2f}s "
