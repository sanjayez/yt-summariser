...
from ai_utils.models import ChatMessage, ChatRequest, ChatRole
...
IMPORTANT: Respond with ONLY the JSON object. No additional text or explanations."""
    ...
    async def enhance_query(
        self, user_query: str, job_id: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            logger.debug(
                f"  Messages: {[{'role': msg.role.value, 'content': msg.content[:100] + '...' if len(msg.content) > 100 else msg.content} for msg in chat_request.messages]}"
            )
            ...
            if result["status"] == "completed":
                ...
                logger.info(
                    f"Query processing metrics - Time: {processing_time}ms, Tokens: {tokens_used}"
                )
                ...
                try:
                    ...
                    if not cleaned_response:
                        logger.error(
                            "No valid JSON found in response - cleaned_response is empty"
                        )
                        raise json.JSONDecodeError(
                            "No valid JSON found in response", llm_response, 0
                        )
                    ...
                    if not isinstance(enhanced_queries, list):
                        enhanced_queries = (
                            [str(enhanced_queries)] if enhanced_queries else []
                        )

                    logger.info(
                        f"Parsed response - Intent: {intent_type}, Concepts: {concepts}, Enhanced: {enhanced_queries}"
                    )
                    ...
                    logger.info(
                        f"Query complexity - Words: {query_length}, Concepts: {num_concepts}, Enhanced queries: {num_enhanced}"
                    )

                    # Validate extracted data quality
                    if not enhanced_queries or all(
                        not q.strip() for q in enhanced_queries
                    ):
                        logger.warning(
                            f"Enhanced queries list is empty or invalid for input: {user_query}"
                        )
                    ...
                    valid_intents = [
                        ...
                        "REVIEW",
                        "HOW_TO",
                    ]  # Match database INTENT_CHOICES exactly
                    if intent_type not in valid_intents:
                        logger.warning(
                            f"Invalid intent '{intent_type}' for query: {user_query}. Defaulting to LOOKUP"
                        )
                        intent_type = "LOOKUP"
                    ...
                    if not concepts or all(not c.strip() for c in concepts):
                        logger.warning(
                            f"Concepts list is empty or invalid for input: {user_query}"
                        )
                    ...
                    enhanced_queries = [
                        q.strip() for q in enhanced_queries if q.strip()
                    ]
                    if not enhanced_queries:
                        logger.warning(
                            "Enhanced queries list is empty, creating fallback queries"
                        )
                    ...
                    enhanced_queries = [
                        ...
                        if q.lower().strip() != original_lower
                    ]
                    if not enhanced_queries:
                        logger.warning(
                            "All enhanced queries were identical to original, creating new ones"
                        )
                        enhanced_queries = self._create_fallback_queries(user_query)
                    ...
                    logger.error(
                        f"JSON parse/validation error for query '{user_query}': {e}"
                    )
                    logger.error(
                        f"Raw LLM response that failed to parse: {llm_response}"
                    )
                    logger.error(f"Exception type: {type(e).__name__}")
                    logger.error(f"Exception details: {str(e)}")
                    logger.warning("Using intelligent fallback analysis.")
                    ...
                    logger.debug(
                        f"  Starting fallback analysis for query: '{user_query}'"
                    )

                    # Intelligent fallback with semantic analysis
                    concepts, enhanced_queries, intent_type, is_complex = (
                        self._intelligent_fallback_analysis(user_query)
                    )

                    logger.debug("FALLBACK RESULTS:")
                ...
                # LLM service failed
                logger.error(
                    f"LLM service failed with status '{result['status']}' for query enhancement: {result.get('error', 'Unknown error')}"
                )
                ...
                logger.debug(
                    f"  Error message: {result.get('error', 'No error message provided')}"
                )
                ...
                logger.debug("LLM SERVICE FAILURE FALLBACK RESULTS:")
                logger.debug(f"  Concepts: {concepts}")
                logger.debug(f"  Enhanced queries: {enhanced_queries}")
                logger.debug(f"  Intent type: {intent_type}")
                logger.debug(f"  Is complex: {is_complex}")
    ...
    def _intelligent_fallback_analysis(
        self, user_query: str
        ...
        for delimiter in concept_delimiters:
            if delimiter in query_lower:
                parts = [
                    part.strip() for part in user_query.split(delimiter) if part.strip()
                ]
                if len(parts) > 1:
                    concepts = parts
                    break
        ...
        for concept in concepts:
            enhanced_query = self._generate_enhanced_query(
                concept, detected_domain, intent_type
            )
        ...
        logger.info(
            f"Fallback analysis - Domain: {detected_domain}, Intent: {intent_type}, Concepts: {len(concepts)}, Complex: {is_complex}"
        )
    ...
    def _extract_basic_concepts(self, user_query: str) -> list[str]:
        ...
        for i in range(len(words)):
            ...
            if i < len(words) - 1:
                ...
                if any(
                    keyword in phrase_clean for keyword in tech_words + domain_words
                ):
    ...
    def _create_fallback_queries(self, user_query: str) -> list[str]:
        ...
        if any(
            word in query_lower for word in ["best", "review", "compare", "vs", "which"]
        ):
    ...
    def _extract_json_from_response(self, response: str) -> str:
        ...
        if "REASONING:" in response:
            ...
            if len(parts) > 1:
                ...
                json_match = re.search(
                    r'\{[\s\S]*?"intent"[\s\S]*?"concepts"[\s\S]*?"enhanced_queries"[\s\S]*?\}',
                    after_reasoning,
                    re.DOTALL,
                )
                if json_match:
        ...
        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
            ...
            if (
                '"intent"' in potential_json
                and '"concepts"' in potential_json
                and '"enhanced_queries"' in potential_json
            ):
        ...
        if (
            ...
            and '"concepts"' in response
            and '"enhanced_queries"' in response
        ):
    ...
    async def _llm_with_retry(
        self, chat_request: ChatRequest, job_id: str, max_retries: int = 2
    ) -> dict[str, Any]:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                result = await self.llm_service.chat_completion(
                    chat_request, job_id=f"{job_id}_attempt_{attempt}"
                )
                ...
                if result["status"] == "completed":
                    ...
                    logger.warning(
                        f"LLM call failed on attempt {attempt + 1}: {result.get('error', 'Unknown error')}"
                    )
                ...
                if attempt == 0 and chat_request.model.startswith("o1"):
                    ...
                    logger.info("Retrying with o1-mini as final attempt")
    ...
    async def batch_enhance_queries(
        self, queries: list[str], job_id: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for i, query in enumerate(queries):
                try:
                    result = await self.enhance_query(
                        query, job_id=f"{job_id}_item_{i}"
                    )
                    results.append(
                        {
                            "index": i,
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unhealthy", "service": "QueryProcessor", "error": str(e)}
