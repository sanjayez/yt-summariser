...
class SearchRequest(BaseModel):
    ...
    intent_type: str | None = Field(
        default=None,
        description="Classified intent type (LOOKUP, TUTORIAL, HOW_TO, REVIEW)",
    )
...
class SearchResponse(BaseModel):
    ...
    metadata: list[SearchResult] | None = Field(
        default=None, description="Optional metadata for results"
    )
...
class YouTubeSearchService:
    ...
    def __init__(self, provider: SearchProvider | None = None):
        ...
        logger.info(
            f"YouTube search service initialized with provider: {self.provider.__class__.__name__}"
        )

    def _wrap_scrapetube_provider(
        self, scrapetube_provider: ScrapeTubeProvider
    ) -> SearchProvider:
        """
        Wrap ScrapeTubeProvider to implement SearchProvider interface
    ...
    def search(self, request: SearchRequest) -> SearchResponse:
        ...
        logger.info(
            f"Searching YouTube for: '{request.query}' (max_results: {request.max_results})"
        )
    ...
    def search_simple(self, query: str, max_results: int = 5) -> list[str]:
        ...
        request = SearchRequest(
            query=query, max_results=max_results, include_metadata=False
        )
    ...
    def configure_provider(self, provider: SearchProvider) -> None:
        ...
        logger.info(
            f"Search provider changed from {old_provider} to {provider.__class__.__name__}"
        )
...
def create_youtube_search_service(
