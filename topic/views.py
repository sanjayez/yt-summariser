...
from topic.serializers import (
    TopicSearchRequestSerializer,
    TopicSearchResponseSerializer,
)
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Dispatched async task {task.id} for search request {search_request.search_id}"
                )
            except Exception as e:
            ...
            if response_serializer.is_valid():
                logger.info(
                    f"Topic search initiated for query: '{query}' with task {task.id}"
                )
                return Response(
                ...
                logger.error(
                    f"Response validation failed: {response_serializer.errors}"
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                logger.info(
                    f"Created search request {search_request.search_id} for query: '{query}'"
                )
                ...
                if search_request.video_urls:
                    ...
                    for url in video_urls:
                        try:
                            ...
                            url_serializer = URLRequestTableSerializer(
                            ...
                            if url_serializer.is_valid():
                                ...
                                logger.info(
                                    f"Created video request {url_request.request_id} for URL: {url}"
                                )
            ...
            with transaction.atomic():
                ...
                if process_videos:
                    ...
                    task = process_search_with_videos.delay(
                        str(search_request.search_id), max_videos
                    )
                    logger.info(
                        f"Started integrated search and video processing task {task.id}"
                    )
                else:
                    # Search only
                    task = process_search_query.delay(str(search_request.search_id))
                ...
                # Serialize response
                response_serializer = SearchToProcessResponseSerializer(
                    data=response_data
                )
                if response_serializer.is_valid():
                    return Response(
...
async def search_status_stream(request, search_id):
    ...
    async def event_stream():
        ...
        logger.info(
            f"Starting enhanced async SSE stream for search request {search_id}"
        )
        ...
        try:
            ...
            try:
                ...
                while True:
                    ...
                    if time.time() - start_time > max_duration:
                        yield f"data: {
                            json.dumps(
                                {'type': 'timeout', 'message': 'Stream timeout reached'}
                            )
                        }\n\n"
                        break
                ...
                logger.warning(
                    f"Redis pub/sub error, falling back to polling: {redis_error}"
                )
                pubsub.close()
                raise redis_error
            finally:
            ...
            max_polls = (
                150  # Maximum number of polling attempts (5 minutes at 2s intervals)
            )
            poll_count = 0
            ...
            while poll_count < max_polls:
                ...
                try:
                    ...
                    yield f"data: {
                        json.dumps(
                            {'type': 'error', 'message': 'Search request not found'}
                        )
                    }\n\n"
                    break
                ...
                url_requests = (
                    URLRequestTable.objects.filter(search_request=search_request)
                ...
                for url_request in url_requests:
                    ...
                    if url_request.status in ["success", "failed"]:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error getting progress for task {url_request.celery_task_id}: {e}"
                            )
                ...
                overall_progress = (
                    (total_progress / video_count) if video_count > 0 else 0
                )
