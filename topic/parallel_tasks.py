...
def monitor_search_progress(self, search_id: str, url_request_ids: list):
    ...
    """
    logger.info(
        f"Starting dedicated progress monitoring task for search {search_id} with {len(url_request_ids)} videos"
    )
    logger.error(
        f"🔴 MONITOR DEBUG: url_request_ids = {url_request_ids[:3]}..."
    )  # Show first 3
    ...
    try:
        ...
        logger.info(
            "Progress monitoring task initialized - beginning event-driven stage tracking"
        )
        ...
        logger.info(
            f"Progress monitoring completed successfully for search {search_id}"
        )
    ...
    try:
        ...
        try:
            ...
            if search_request.status != "success":
                ...
                logger.error(error_msg)
                progress.send_error(
                    f"Search not ready for processing: {search_request.status}"
                )
                return {
                    "status": "failed",
            ...
            logger.info(
                f"Processing {len(video_urls)} videos for search request {search_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Created {len(url_request_ids)} URLRequestTable entries for search request {search_id}"
            )
            ...
            _update_search_request_processing_status(
                search_request, "failed", error_msg
            )
            return {
                "status": "failed",
        ...
        try:
            ...
            logger.info(
                f"Initiated parallel processing with {len(task_ids)} tasks for search {search_id}"
            )
        ...
        try:
            logger.info(
                f"Launching dedicated progress monitoring task for {len(url_request_ids)} videos"
            )
            ...
            logger.info(
                f"Video processing orchestration completed for search {search_id}"
            )
            ...
            try:
                ...
                progress.expedition_complete()
                logger.info("Sent fallback completion due to monitoring launch failure")
            except Exception as fallback_error:
                logger.error(f"Fallback completion failed: {fallback_error}")
        ...
        if self.request.retries >= self.max_retries:
            handle_dead_letter_task(
                "process_search_results", self.request.id, [search_id], {}, e
            )

        try:
            if search_request:
                _update_search_request_processing_status(
                    search_request, "failed", f"Unexpected error: {str(e)}"
                )
            if session:
                update_session_status(session, "failed")
        except:
...
def finalize_search_processing(
    self, processing_results: list[Any], search_id: str, url_request_ids: list[str]
):
    """
    ...
    """
    logger.info(
        f"Finalizing processing for search request {search_id} with {len(url_request_ids)} videos"
    )
    ...
    try:
        ...
        try:
            with transaction.atomic():
                _update_search_request_processing_status(
                    search_request, final_status, status_message
                )
                ...
                if search_request.status == "failed":
                    ...
                    update_session_status(
                        session, "success"
                    )  # Keep success regardless of video processing
                # Keep session as processing if search is still processing

                logger.info(
                    f"Search request {search_id} finalized with video processing status: {final_status}"
                )
                logger.info(
                    f"SearchRequest.status remains: {search_request.status} (search success)"
                )

        except Exception as e:
            logger.error(
                f"Failed to update final status for search request {search_id}: {e}"
            )
        ...
        logger.error(
            f"Finalization soft timeout reached for search {search_id} - database issue"
        )
        ...
        try:
            ...
            logger.critical(
                f"Failed to emergency update status during finalization timeout: {cleanup_error}"
            )
...
def _create_url_request_entries(
    search_request: SearchRequestModel, video_urls: list[str]
) -> list[str]:
    """
    ...
    if excluded_info:
        logger.info(
            f"Pre-filtered {len(excluded_info)} excluded videos for search {search_request.search_id}"
        )
    ...
    if not processable_urls:
        logger.warning(
            f"No processable videos after pre-filtering for search {search_request.search_id}"
        )
        return []

    logger.info(
        f"Creating URLRequestTable entries for {len(processable_urls)} processable videos "
    ...
    with transaction.atomic():
        URLRequestTable.objects.bulk_create(url_request_entries)
        ...
        created_ids = list(
            ...
            )  # Use 'request_id' UUID for URLRequestTable
        )
...
def _analyze_processing_results(url_request_ids: list[str]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "success_rate": (successful_videos / total_videos) * 100
            if total_videos > 0
            else 0,
        }
...
def _determine_final_status(processing_stats: dict[str, Any]) -> str:
    ...
    if processing_stats["processing_videos"] > 0:
        ...
        success_rate = (
            processing_stats["success_rate"] / 100
        )  # Convert percentage to decimal
        minimum_threshold = BUSINESS_LOGIC_CONFIG["PIPELINE_SUCCESS"][
            "minimum_threshold"
        ]
...
def _generate_status_message(processing_stats: dict[str, Any]) -> str:
    ...
    if processing > 0:
        return (
            f"Processing {processing} videos ({successful} completed, {failed} failed)"
        )
    elif successful == total:
        return f"Successfully processed all {total} videos"
    elif successful > 0:
        return (
            f"Processed {successful} of {total} videos successfully ({failed} failed)"
        )
    else:
        return f"Failed to process all {total} videos"


def _update_search_request_processing_status(
    search_request: SearchRequestModel, status: str, message: str
):
    """
