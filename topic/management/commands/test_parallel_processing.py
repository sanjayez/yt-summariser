...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--timeout", type=int, default=300, help="Timeout in seconds for waiting"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Starting parallel processing test"))
    ...
    def _create_test_data(self, query):
        ...
        with transaction.atomic():
            session = SearchSession.objects.create(
                user_ip="127.0.0.1", status="processing"
            )
            ...
            self.stdout.write(
                f"Created test data - Session: {session.session_id}, Request: {search_request.search_id}"
            )
    ...
    def _test_parallel_processing(self, search_request):
        ...
        search_result = process_search_query.apply_async(
        ...
        result = process_search_results.apply_async(
            args=[str(search_request.search_id)]
        )
    ...
    def _wait_for_completion(self, search_request, timeout):
        ...
        self.stdout.write(
            self.style.WARNING(f"Waiting for completion (timeout: {timeout}s)")
        )
    ...
    def _print_final_results(self, search_request):
        ...
        if video_results.get("status") == "success":
            ...
            self.stdout.write(
                f"Video Processing Summary: {json.dumps(summary, indent=2)}"
            )
