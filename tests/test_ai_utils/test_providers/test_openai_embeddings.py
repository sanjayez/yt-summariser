...
from ai_utils.models import BatchEmbeddingRequest, EmbeddingRequest
...
class TestOpenAIEmbeddingProvider:
    ...
    def provider(self, mock_config, mock_openai_client):
        ...
        with patch(
            "ai_utils.providers.openai_embeddings.AsyncOpenAI",
    ...
    async def test_embed_text_success(self, provider):
        ...
        request = EmbeddingRequest(
            text="Hello world", model="text-embedding-3-small", user="test_user"
        )
    ...
    async def test_embed_batch_success(self, provider):
        ...
        request = BatchEmbeddingRequest(
            texts=["Hello", "World"], model="text-embedding-3-small", user="test_user"
        )
    ...
    async def test_get_embedding_with_retry_success(self, provider):
        ...
        result = await provider.get_embedding_with_retry(
    ...
    async def test_get_embedding_with_retry_failure(self, provider):
        ...
        with pytest.raises(Exception, match="API Error"):
            await provider.get_embedding_with_retry(
                "Hello world", model="text-embedding-3-small", max_retries=2
            )
    ...
    async def test_get_embedding_with_retry_success_after_failure(self, provider):
        ...
        result = await provider.get_embedding_with_retry(
            "Hello world", model="text-embedding-3-small", max_retries=3
        )
