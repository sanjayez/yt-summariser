...
from video_processor.models import TranscriptSegment, VideoMetadata, VideoTranscript
...
class PrimaryKeyMigrationTests(TestCase):
    ...
    def test_natural_key_managers(self):
        ...
        retrieved_transcript = VideoTranscript.objects.get_by_video_id(
            self.test_video_id
        )
    ...
    def test_cascade_deletion(self):
        ...
        self.assertFalse(
            VideoMetadata.objects.filter(video_id=self.test_video_id).exists()
        )
        self.assertFalse(
            VideoTranscript.objects.filter(video_id=self.test_video_id).exists()
        )
        self.assertFalse(
